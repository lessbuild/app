...
final class ReconcileProjectConnectionDeliveriesCommand extends Command
{
    ...
    public function handle(
        ...
        $sources = $source === null ? ['deployer', ...(config('platform.products.monitor.enabled', false) ? ['monitor'] : [])] : [$source];
        ...
        foreach ($sources as $product) {
            ...
            $table = $product === 'deployer'
                ? 'deployment_succeeded_outbox_events'
                : 'project_connection_incident_outbox_events';
            $connection = $product === 'deployer' ? 'deployer' : 'monitor';
        ...
        $events = $this->sourceEvents($sources, $eventId, $limit);
        ...
        foreach ($events as [$product, $event]) {
            try {
                if ($product === 'deployer') {
                    $missing = $deployerDispatcher->missingDeliveryCount($event);
                    $created = $this->option('apply') ? $deployerDispatcher->dispatch($event) : 0;
                } else {
                    $missing = $monitorDispatcher->missingDeliveryCount($event);
                    $created = $this->option('apply') ? $monitorDispatcher->dispatch($event) : 0;
                }
            } catch (Throwable $exception) {
                $hasErrors = true;
                $rows[] = [$product, (string) $event->getKey(), $event->status, 'error: '.$exception::class];
        ...
        if ($this->option('apply')) {
            ...
            $this->info("Read-only preview: {$totalMissing} missing Core delivery record(s) across {$events->count()} source event(s). Pass --apply to create only these eligible records.");
        }
     ...
     * @return Collection<int, array{0: string, 1: Model}>
     */
    private function sourceEvents(array $sources, ?string $eventId, int $limit): Collection
    {
        $events = collect();
        ...
        foreach ($sources as $source) {
            ...
            $query->whereIn('status', ['dispatched', 'failed']);
        ...
        return $events
            ->sortBy(static fn (array $entry): string => $entry[1]->created_at?->toISOString() ?? '')
            ->take($limit)
            ->values();
    }
}
