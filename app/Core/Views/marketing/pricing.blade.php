...
>
    ...
    <main id="main-content" tabindex="-1" class="mx-auto max-w-screen-2xl px-5 py-12 sm:px-8 sm:py-16">
        ...
        <section aria-labelledby="deployer-pricing-title" class="mt-12" x-data="{ interval: 'yearly' }" data-product-pricing="deployer">
            ...
            <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($deployerPlans as $key => $plan)
                    ...
                    <x-signal.ui.card class="relative flex flex-col p-6 sm:p-7" data-pricing-plan="deployer-{{ $key }}" @class(['border-2 ring-2' => $isFeatured]) @style(['border-color: var(--ui-primary); --tw-ring-color: var(--ui-primary)' => $isFeatured])>
                        @if ($isFeatured)
                        ...
                        <p class="mt-2 min-h-12 text-sm leading-6 text-muted">{{ $plan['description'] }}</p>
                        ...
                        <x-signal.ui.card as="dl" tone="muted" :shadow="false" class="grid grid-cols-2 gap-x-3 gap-y-2 p-4 text-xs">
        ...
        <section aria-labelledby="monitor-pricing-title" class="mt-16" data-product-pricing="monitor">
            <div class="mb-6">
            ...
            <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                @foreach ($monitorPlans as $key => $plan)
                    @php($isFeatured = $key === 'pro')
                    <x-signal.ui.card class="relative flex flex-col p-6" data-pricing-plan="monitor-{{ $key }}" @class(['border-2 ring-2' => $isFeatured]) @style(['border-color: var(--ui-primary); --tw-ring-color: var(--ui-primary)' => $isFeatured])>
                        @if ($isFeatured)
        ...
        <section aria-labelledby="analytics-pricing-title" class="mt-16" data-product-pricing="analytics">
            <x-signal.ui.card class="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
