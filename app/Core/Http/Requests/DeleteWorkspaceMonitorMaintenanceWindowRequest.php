...
final class DeleteWorkspaceMonitorMaintenanceWindowRequest extends WorkspaceMonitorAdministrationRequest
{
    ...
    {
        return [
            'window_reference' => ['required', 'string', 'max:4096'],
            'version' => ['required', 'string', 'regex:/\A\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?:\.\d{1,6})?\z/'],
            'confirm_remove' => ['required', 'accepted'],
        ];
    }
}
