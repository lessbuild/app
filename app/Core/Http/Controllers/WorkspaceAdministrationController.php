...
final class WorkspaceAdministrationController
{
    ...
    public function __invoke(
        ...
        PlatformProductRouteLinks $links,
    ): View {
        ...
        foreach (self::MODULE_TOOLS as $product => $tools) {
            if (! $access->hasProductAccess($membership, $product)) {
                continue;
            }

            $moduleTools[$product] = collect($tools)->map(function (array $tool) use ($product, $links): ?array {
                $href = $links->to($product, $tool['route']);
                if ($href === null) {
                    return null;
                }
