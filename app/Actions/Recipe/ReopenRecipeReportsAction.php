...
class ReopenRecipeReportsAction
{
    ...
    /**
     * Reopen a sorted, contributor-owned report selection atomically and record one audit event per recipe.
     *
     ...
     * @return int The number of reports that changed from resolved to open.
     *
    ...
    public function handle(User $contributor, array $reportIds): int
    {
        return DB::transaction(function () use ($contributor, $reportIds): int {
            ...
            RecipeReport::query()
                ...
                ->with(['recipe:id,user_id,name'])
                ->get()
                ->each(function (RecipeReport $report): void {
                    $this->notifications->open($report->recipe, $report);
                    $this->notifications->reopened($report->recipe, $report);
                });

            $resolved->groupBy('recipe_id')->each(function ($reports) use ($contributor): void {
