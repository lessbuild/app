...
class GitHubPreviewReporter
{
    ...
    public function report(PreviewDeployment $preview): void
    {
        ...
        $summary = match ($preview->status) {
            ...
            PreviewDeployment::STATUS_FAILED => 'Preview deployment failed. Open '.config('app.name', 'Deployer').' to inspect the deployment log.',
            PreviewDeployment::STATUS_CLOSED => 'Preview environment has been closed.',
            default => config('app.name', 'Deployer').' is preparing an isolated preview environment.',
        };
        try {
