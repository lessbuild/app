<x-layouts.app>
    ...
    >
        <dl class="ui-insight-grid grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
            <x-signal.ui.stat :label="__('Matching alerts')" :value="$metrics['total']" :description="__('Alerts in this filtered view.')" />
            <x-signal.ui.stat :label="__('Unread alerts')" :value="$metrics['unread']" :description="__('Matching alerts still awaiting review.')" />
            <x-signal.ui.stat :label="__('Failures')" :value="$metrics['failed']" :description="__('Matching failed incidents.')" />
            <x-signal.ui.stat :label="__('Recoveries')" :value="$metrics['healthy']" :description="__('Matching recovery notices.')" />
            <x-signal.ui.stat :label="__('Information')" :value="$metrics['info']" :description="__('Matching informational notices.')" />
            <x-signal.ui.stat :label="__('Latest matching alert')" :value="$metrics['latest_at']?->diffForHumans() ?? __('Not available')" :description="$metrics['latest_at']?->toDayDateTimeString() ?? __('No matching alert recorded.')" />
        </dl>
    </x-signal.ui.insights>
    ...
    >
        @if ($notifications->isNotEmpty())
            <x-signal.ui.panel as="form" id="notification-bulk-form" method="POST" action="{{ route('notifications.bulk') }}" class="ui-panel sticky top-3 z-10 flex flex-wrap items-center gap-2 border-l-4 p-3" style="border-left-color: var(--ui-primary)">
                @csrf
                @method('PATCH')
                <x-signal.ui.button type="button" variant="secondary" x-on:click="selected = selected.length === pageIds.length ? [] : [...pageIds]">
                ...
                <x-signal.ui.button type="submit" name="action" value="read" variant="secondary" x-bind:disabled="selected.length === 0">{{ __('Mark read') }}</x-signal.ui.button>
                <x-signal.ui.button type="submit" name="action" value="unread" variant="secondary" x-bind:disabled="selected.length === 0">{{ __('Mark unread') }}</x-signal.ui.button>
                <x-signal.ui.button type="submit" name="action" value="delete" variant="danger" x-bind:disabled="selected.length === 0" onclick="return confirm({{ Illuminate\Support\Js::from(__('Delete the selected notifications? This cannot be undone.')) }})">{{ __('Delete selected') }}</x-signal.ui.button>
            </x-signal.ui.panel>
        ...
        <x-signal.ui.panel id="notification-list" class="ui-panel ui-inventory-list scroll-mt-24 overflow-hidden p-4 sm:p-6">
        <div class="space-y-3">
        @forelse ($notifications as $notification)
            @php
                ...
                $notificationStatus = $notification->data['status'] ?? \App\Modules\Deployer\Notifications\NotificationInbox::STATUS_FAILED;
                $notificationTone = match ($notificationStatus) {
                    \App\Modules\Deployer\Notifications\NotificationInbox::STATUS_HEALTHY => 'success',
                    \App\Modules\Deployer\Notifications\NotificationInbox::STATUS_INFO => 'accent',
                    default => 'danger',
                };
            @endphp
            <article
            ...
            >
                <div class="flex items-start gap-3">
                    <x-signal.ui.input type="checkbox" name="notifications[]" value="{{ $notification->id }}" form="notification-bulk-form" x-model="selected" class="ui-check mt-1 shrink-0" aria-label="{{ __('Select notification: :title', ['title' => $notification->data['title'] ?? __('Notification')]) }}" :restore="false" />
                    @if ($notification->read_at !== null)
                        <details id="notification-{{ $notification->id }}" class="group min-w-0 flex-1">
                            <summary class="flex cursor-pointer list-none flex-wrap items-center gap-2 rounded-control focus:outline-none focus-visible:ring-2 focus-visible:ring-focus">
                                ...
                                <x-signal.ui.badge>{{ __('Read') }}</x-signal.ui.badge>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4 flex flex-wrap gap-2">
                                    @if ($destinationState !== null && $destinationState['available'])
                                        <x-signal.ui.button :href="$destination" variant="secondary">{{ __('View') }}</x-signal.ui.button>
                                    @elseif ($destinationUnavailable)
                                        <x-signal.ui.button :href="$destinationState['fallback']" variant="secondary">{{ __('Open related inventory') }}</x-signal.ui.button>
                                    @endif
                                    <form method="POST" action="{{ route('notifications.unread', $notification->id) }}">
                                        @csrf
                                        <x-signal.ui.button type="submit" variant="ghost">{{ __('Mark unread') }}</x-signal.ui.button>
                                    </form>
                    ...
                    @else
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <x-signal.ui.badge :tone="$notificationTone">{{ __('Unread') }}</x-signal.ui.badge>
                            </div>
                            ...
                            <div class="mt-4 flex flex-wrap gap-2">
                                <form method="POST" action="{{ route('notifications.read', $notification->id) }}">
                                    @csrf
                                    <x-signal.ui.button type="submit" variant="secondary">{{ $destinationState !== null && $destinationState['available'] ? __('View and mark read') : __('Mark as read') }}</x-signal.ui.button>
                                </form>
                                @if ($destinationUnavailable)
                                    <x-signal.ui.button :href="$destinationState['fallback']" variant="ghost">{{ __('Open related inventory') }}</x-signal.ui.button>
                                @endif
                                <form method="POST" action="{{ route('notifications.destroy', $notification->id) }}" onsubmit="return confirm({{ Illuminate\Support\Js::from(__('Delete this notification?')) }})">
                                    @csrf
                                    @method('DELETE')
    ...
    <x-signal.ui.panel as="section" class="ui-panel mb-6 mt-8 p-4" aria-labelledby="notification-tools">
        ...
        <details id="notification-filters" @if ($filtersAreActive) open @endif>
            <summary class="flex cursor-pointer list-none items-center justify-between gap-3 rounded-control font-bold text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-focus">
                ...
                <span class="flex items-center gap-2">
                    @if ($filtersAreActive)
                        <x-signal.ui.badge tone="accent">{{ trans_choice(':count active|:count active', $activeFilterCount, ['count' => $activeFilterCount]) }}</x-signal.ui.badge>
                    @endif
            ...
            <form method="GET" action="{{ route('notifications.index') }}" class="mt-4">
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
                    <div>
                        ...
                        <x-signal.ui.input id="search" name="search" type="search" maxlength="100" value="{{ $filters['search'] }}" placeholder="{{ __('Notification title or message') }}" class="ui-input" :restore="false" />
                    </div>
                    <div>
                        <label for="category" class="ui-label">{{ __('Category') }}</label>
                        <x-signal.ui.select id="category" name="category" class="ui-input"><option value="">{{ __('All categories') }}</option>@foreach ($categories as $category)<option value="{{ $category }}" @selected($filters['category'] === $category)>{{ str($category)->title() }}</option>@endforeach</x-signal.ui.select>
                    </div>
                    <div>
                        <label for="status" class="ui-label">{{ __('Status') }}</label>
                        <x-signal.ui.select id="status" name="status" class="ui-input"><option value="">{{ __('All statuses') }}</option><option value="failed" @selected($filters['status'] === 'failed')>{{ __('Failed') }}</option><option value="healthy" @selected($filters['status'] === 'healthy')>{{ __('Recovered') }}</option><option value="info" @selected($filters['status'] === 'info')>{{ __('Information') }}</option></x-signal.ui.select>
                    </div>
                    <div>
                        <label for="state" class="ui-label">{{ __('State') }}</label>
                        <x-signal.ui.select id="state" name="state" class="ui-input"><option value="">{{ __('Read and unread') }}</option><option value="unread" @selected($filters['state'] === 'unread')>{{ __('Unread') }}</option><option value="read" @selected($filters['state'] === 'read')>{{ __('Read') }}</option></x-signal.ui.select>
                    </div>
                    <div>
                ...
                <div class="mt-4 flex flex-wrap gap-3">
                    ...
                    <x-signal.ui.button :href="route('notifications.export', array_filter($filters, fn ($value) => $value !== null))" variant="secondary">{{ __('Export CSV') }}</x-signal.ui.button>
                    @if ($filtersAreActive)
                        <x-signal.ui.button :href="route('notifications.index')" variant="ghost">{{ __('Clear filters') }}</x-signal.ui.button>
                    @endif
                </div>
            </form>
        </details>
    ...
    <x-signal.ui.panel as="section" class="ui-panel mb-6 p-4" aria-labelledby="saved-notification-filters">
        <details id="notification-saved-filters" @if ($errors->has('name')) open @endif>
            <summary class="flex cursor-pointer list-none items-center justify-between gap-3 rounded-control font-bold text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-focus">
                <span>{{ __('Saved filters') }}</span>
                <span class="flex items-center gap-2">
                    @if ($savedFilters)
                        <x-signal.ui.badge>{{ count($savedFilters) }}</x-signal.ui.badge>
                    @endif
                    <span class="text-muted" aria-hidden="true">⌄</span>
                </span>
            </summary>
            <div class="mt-3 flex flex-wrap items-start justify-between gap-4">
                ...
                <x-signal.ui.button
                    ...
                    variant="secondary"
                >{{ __('Save current') }}</x-signal.ui.button>
            </div>
            @if ($savedFilters)
                <div class="mt-4 flex flex-wrap gap-2">
                    @foreach ($savedFilters as $saved)
                        <div class="ui-chip overflow-hidden p-0">
                            ...
                            <form method="POST" action="{{ route('notifications.saved-filters.destroy', $saved['id']) }}">
                                @csrf
                                @method('DELETE')
                                <x-signal.ui.button variant="link" type="submit" class="px-3 py-2 text-muted hover:bg-surface hover:text-ink" aria-label="{{ __('Remove saved filter :name', ['name' => $saved['name']]) }}">×</x-signal.ui.button>
                            </form>
                        </div>
                    @endforeach
                </div>
