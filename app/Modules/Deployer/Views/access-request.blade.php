                        ...
                        <form method="POST" action="{{ route('access-request.store') }}" class="mt-6 grid gap-5 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <x-signal.ui.select id="access-team-size" name="team_size" class="ui-input">
                                    ...
                                    @foreach (\App\Modules\Deployer\Models\AccessRequest::TEAM_SIZES as $size)
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <x-signal.ui.textarea id="access-use-case" name="use_case" required minlength="20" maxlength="2000" rows="6" class="ui-input" placeholder="{{ __('Current stack, provider, number of servers, and the problem you want :app to solve.', ['app' => config('app.name')]) }}" :restore="false">{{ old('use_case') }}</x-signal.ui.textarea>
                                <span class="ui-help">{{ __('Please do not include passwords, tokens, or other secrets.') }}</span>
                            </div>

                            <div class="hidden" aria-hidden="true"><label>Website<x-signal.ui.input name="website" tabindex="-1" autocomplete="off" :restore="false" /></label></div>

                            <x-signal.ui.button type="submit" variant="primary" class="w-full sm:col-span-2">
