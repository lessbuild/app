<div @if ($shouldPoll) wire:poll.5s @endif>
    @php
        ...
        $statusTone = match ($build->status) {
            ...
            \App\Modules\Deployer\Models\Build::STATUS_FAILED, \App\Modules\Deployer\Models\Build::STATUS_REJECTED => 'danger',
            \App\Modules\Deployer\Models\Build::STATUS_CANCELED => 'warning',
            \App\Modules\Deployer\Models\Build::STATUS_RUNNING, \App\Modules\Deployer\Models\Build::STATUS_QUEUED, \App\Modules\Deployer\Models\Build::STATUS_AWAITING_APPROVAL, \App\Modules\Deployer\Models\Build::STATUS_TIMING_OUT => 'accent',
            default => 'neutral',
        };
    @endphp
        ...
        <dl class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-ink">
                    ...
                    @else
                        {{ $build->trigger_source === \App\Modules\Deployer\Models\Build::TRIGGER_WEBHOOK ? __('Source webhook') : __('No actor recorded') }}
                    @endif
                </dd>
            </div>
            <div>
                ...
                <dd class="mt-1 text-sm text-ink">
                    ...
                    @elseif ($build->status === \App\Modules\Deployer\Models\Build::STATUS_AWAITING_APPROVAL)
    ...
    @if ($build->status === \App\Modules\Deployer\Models\Build::STATUS_SUCCEEDED && $build->release_name && $build->release_path && $build->trigger_source !== \App\Modules\Deployer\Models\Build::TRIGGER_ROLLBACK)
        @can('rollback', $build)
