        ...
        <x-signal.ui.panel as="section" id="context-deployments" class="ui-panel p-5 sm:p-6" aria-labelledby="context-deployments-heading" data-observability-context-section>
            ...
            <div class="ui-inventory-list mt-4 space-y-2">
                @forelse($context->builds as $build)
                    @php
                        $buildColor = in_array($build->status, [\App\Modules\Deployer\Models\Build::STATUS_FAILED, \App\Modules\Deployer\Models\Build::STATUS_CANCELED], true)
                            ? 'var(--ui-danger)'
                            : ($build->status === \App\Modules\Deployer\Models\Build::STATUS_SUCCEEDED ? 'var(--ui-success)' : 'var(--ui-warning)');
                        $observation = $context->deploymentObservations->get((int) $build->id);
                        $observationTone = match ($observation?->status) {
                            \App\Modules\Deployer\Models\DeploymentObservation::STATUS_HEALTHY => 'success',
                            \App\Modules\Deployer\Models\DeploymentObservation::STATUS_FAILED, \App\Modules\Deployer\Models\DeploymentObservation::STATUS_EXPIRED => 'danger',
                            \App\Modules\Deployer\Models\DeploymentObservation::STATUS_PENDING, \App\Modules\Deployer\Models\DeploymentObservation::STATUS_OBSERVING => 'warning',
                            default => 'neutral',
                        };
                    @endphp
