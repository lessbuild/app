                    ...
                    @forelse ($attentionWebsites as $website)
                        <x-signal.ui.card as="a" tone="interactive" class="block p-3" href="{{ route('websites.show', $website) }}">
                            ...
                            <span class="text-sm text-muted">
                                ...
                                @endif
                                @if ($website->provisioning_status === \App\Modules\Deployer\Models\Website::STATUS_FAILED
                                    && $website->health_status === \App\Modules\Deployer\Models\Website::HEALTH_UNHEALTHY)
                                    &middot;
                                @endif
                                @if ($website->health_status === \App\Modules\Deployer\Models\Website::HEALTH_UNHEALTHY)
            ...
            <div>
                ...
                <div class="space-y-2">
                    ...
                    @if ($attentionCounts['servers'] > $attentionServers->count())
                        <a href="{{ route('servers.index', ['status' => \App\Modules\Deployer\Models\Server::STATUS_FAILED]) }}" class="ui-link block text-sm">
            ...
            <div>
                ...
                <div class="space-y-2">
                    ...
                    @if ($attentionCounts['deployments'] > $attentionRepositories->count())
                        <a href="{{ route('builds.index', ['status' => \App\Modules\Deployer\Models\Build::STATUS_FAILED, 'latest' => 1]) }}" class="ui-link block text-sm">
            ...
            <div>
                ...
                <div class="space-y-2">
                    ...
                    @if ($attentionCounts['providers'] > $attentionProviders->count())
                        <a href="{{ route('providers.index', ['connection' => \App\Modules\Deployer\Models\Provider::CONNECTION_FAILED]) }}" class="ui-link block text-sm">
