@php
    $durationComparison = match (true) {
        ...
        $durationDelta > 0 => __(':duration slower', ['duration' => \App\Modules\Deployer\Models\Build::formatDuration($durationDelta)]),
        $durationDelta < 0 => __(':duration faster', ['duration' => \App\Modules\Deployer\Models\Build::formatDuration(abs($durationDelta))]),
                            ...
                            <dd @class(['mt-2 text-ink', 'whitespace-pre-wrap break-words' => $comparison['long'] ?? false, 'font-mono text-xs' => $comparison['revision'] ?? false])>
                                ...
                                @if (($comparison['duration'] ?? false) && $side['build']->is($build))
                                    @if ($durationDelta !== null)
                                        @if ($durationDelta > 0)
                                            <span class="mt-2 block text-xs text-danger">{{ __(':duration slower', ['duration' => \App\Modules\Deployer\Models\Build::formatDuration($durationDelta)]) }}</span>
                                        @elseif ($durationDelta < 0)
                                            <span class="mt-2 block text-xs text-success">{{ __(':duration faster', ['duration' => \App\Modules\Deployer\Models\Build::formatDuration(abs($durationDelta))]) }}</span>
                                        @else
