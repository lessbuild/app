        ...
        <div class="mt-4 border-t border-line pt-4">
            ...
            >
                @foreach (\App\Modules\Deployer\Models\Website::HEALTH_CHECK_INTERVALS as $minutes)
                    <option
                        value="{{ $minutes }}"
                        @selected((int) old('health_check_interval_minutes', $website?->health_check_interval_minutes ?? \App\Modules\Deployer\Models\Website::DEFAULT_HEALTH_CHECK_INTERVAL_MINUTES) === $minutes)
                    >
        ...
        <div class="mt-4 border-t border-line pt-4">
            ...
            >
                @foreach (\App\Modules\Deployer\Models\Website::HEALTH_FAILURE_THRESHOLDS as $failures)
                    <option
                        value="{{ $failures }}"
                        @selected((int) old('health_failure_threshold', $website?->health_failure_threshold ?? \App\Modules\Deployer\Models\Website::defaultHealthFailureThreshold()) === $failures)
                    >
