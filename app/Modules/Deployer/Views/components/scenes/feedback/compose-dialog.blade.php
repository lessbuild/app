    ...
    <form method="POST" action="{{ route('feedback.store') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                ...
                <x-signal.ui.select id="feedback-category" name="category" class="ui-input">
                    @foreach (\App\Modules\Deployer\Models\ProductFeedback::CATEGORIES as $value)
            ...
            <div>
                ...
                <x-signal.ui.select id="feedback-severity" name="severity" class="ui-input">
                    @foreach (\App\Modules\Deployer\Models\ProductFeedback::SEVERITIES as $value)
        ...
        <div>
            ...
            <x-signal.ui.input id="feedback-title" name="title" maxlength="160" required value="{{ old('title') }}" class="ui-input" placeholder="{{ __('What happened or should change?') }}" :restore="false" />
        </div>
        <div>
            <label for="feedback-description" class="ui-label">{{ __('Details') }}</label>
            <x-signal.ui.textarea id="feedback-description" name="description" rows="5" maxlength="10000" required class="ui-input" placeholder="{{ __('Describe the outcome you expected and what you saw.') }}" :restore="false">{{ old('description') }}</x-signal.ui.textarea>
        </div>
        <div>
            <label for="feedback-reproduction" class="ui-label">{{ __('Steps to reproduce (optional)') }}</label>
            <x-signal.ui.textarea id="feedback-reproduction" name="reproduction_steps" rows="4" maxlength="10000" class="ui-input" placeholder="1. Open…" :restore="false">{{ old('reproduction_steps') }}</x-signal.ui.textarea>
        </div>
        <div>
            <label for="feedback-page" class="ui-label">{{ __('Related page (optional)') }}</label>
            <x-signal.ui.input id="feedback-page" name="page" maxlength="500" value="{{ old('page', request()->query('from')) }}" class="ui-input font-mono" placeholder="/projects/12" :restore="false" />
        </div>
