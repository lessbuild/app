...
<section data-activity-audit-content aria-labelledby="account-audit-content-heading">
    ...
    >
        <dl class="ui-insight-grid grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <x-signal.ui.stat class="ui-card" :label="__('Account events')" :value="$metrics['account']" :description="__('Matching account-security changes.')" />
            <x-signal.ui.stat class="ui-card" :label="__('Matching events')" :value="$metrics['total']" :description="__('Owner-scoped events in this view.')" />
    ...
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-3">
            <x-signal.ui.button :href="$fullPageUrl" variant="secondary">{{ __('Open full audit') }}</x-signal.ui.button>
            @if ($auditAvailable)
