                ...
                @foreach ($servers as $server)
                    <article data-recipe-server-assignment class="p-4 transition-colors hover:bg-surface-muted sm:p-5">
                        <div class="flex flex-wrap items-start justify-between gap-4">
                            ...
                            <x-ui.badge :tone="$server->provisioning_status === \App\Modules\Deployer\Models\Server::STATUS_ACTIVE ? 'success' : ($server->provisioning_status === \App\Modules\Deployer\Models\Server::STATUS_FAILED ? 'danger' : 'info')">
