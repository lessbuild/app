    ...
    <div class="mt-8 grid gap-3 md:grid-cols-2">
        @forelse($repositories as $repository)
            <x-signal.ui.card tone="interactive" class="flex items-center gap-4 p-4">
                ...
                <x-signal.ui.button :href="$repositoryCreateUrl" data-modal-trigger="repository-create-dialog" data-modal-content-url="{{ $repositoryCreateContentUrl }}" aria-controls="repository-create-dialog" aria-expanded="{{ $repositoryCreateOpen ? 'true' : 'false' }}" variant="primary">{{ __('Connect') }}</x-signal.ui.button>
            </x-signal.ui.card>
        @empty
            <x-signal.ui.empty-state class="md:col-span-2" :title="__('No repositories available')" :description="__('Update the GitHub App installation and grant access to at least one repository.')" />
        @endforelse
    </div>
</x-layouts.app>
