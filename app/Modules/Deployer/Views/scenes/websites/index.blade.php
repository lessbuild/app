<x-layouts.app>
    ...
    >
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div>
                <x-signal.ui.select-field id="health" name="health" :label="__('Health')" :error-key="false">
            ...
            <div class="flex items-end">
                <x-signal.ui.checkbox id="attention" name="attention" :value="1" :checked="$filters['attention']" :restore="false" :error-key="false" container-class="w-full content-end">
                    {{ __('Needs attention only') }}
                </x-signal.ui.checkbox>
            </div>
            <div class="flex items-end">
                <x-signal.ui.checkbox id="provisioning" name="provisioning" :value="1" :checked="$filters['provisioning']" :restore="false" :error-key="false" container-class="w-full content-end">
        ...
        <div class="mt-4 flex flex-wrap gap-3">
            ...
            <x-signal.ui.button :href="route('websites.export', array_filter($filters, fn ($value) => $value !== null))" variant="secondary">
                {{ __('Export CSV') }}
            </x-signal.ui.button>
            @if (array_filter($filters, fn ($value) => $value !== null))
                <x-signal.ui.button :href="route('websites.index')" variant="ghost">{{ __('Clear filters') }}</x-signal.ui.button>
            @endif
        </div>
        </form>
    </x-signal.ui.filter-panel>
    ...
    >
        <dl class="ui-insight-grid grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
            <x-signal.ui.stat :label="__('Matching websites')" :value="$metrics['total']" :description="__('Websites in this filtered view.')" />
            <x-signal.ui.stat :label="__('Active websites')" :value="$metrics['active']" :description="__('Matching provisioned websites.')" />
            <x-signal.ui.stat :label="__('Provisioning')" :value="$metrics['provisioning']" :description="__('Queued or provisioning websites.')" />
            <x-signal.ui.stat :label="__('Failed websites')" :value="$metrics['failed']" :description="__('Matching provisioning failures.')" />
            <x-signal.ui.stat :label="__('Unhealthy websites')" :value="$metrics['unhealthy']" :description="__('Enabled health checks reporting unhealthy.')" />
            <x-signal.ui.stat :label="__('Needs attention')" :value="$metrics['attention']" :description="__('Provisioning failures or enabled unhealthy checks.')" />
        </dl>
    </x-signal.ui.insights>
    ...
    ! List Websites
     ...
     !-->
    @if(!$websites->isEmpty())
        <x-signal.ui.panel class="ui-inventory-list mt-6 divide-y divide-line overflow-hidden" aria-label="{{ __('Website inventory') }}">
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            @if ($website->provisioning_status === \App\Modules\Deployer\Models\Website::STATUS_ACTIVE)
                                <x-signal.ui.badge tone="success">{{ str($website->provisioning_status)->replace('_', ' ') }}</x-signal.ui.badge>
                            @elseif ($website->provisioning_status === \App\Modules\Deployer\Models\Website::STATUS_FAILED)
                                <x-signal.ui.badge tone="danger">{{ str($website->provisioning_status)->replace('_', ' ') }}</x-signal.ui.badge>
                            @else
                                <x-signal.ui.badge tone="accent">{{ str($website->provisioning_status)->replace('_', ' ') }}</x-signal.ui.badge>
                            @endif
                            <x-signal.ui.button :href="route('websites.show', $website)" variant="secondary">{{ __('View website') }}</x-signal.ui.button>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-ink">
                                ...
                                @elseif ($website->health_status === \App\Modules\Deployer\Models\Website::HEALTH_HEALTHY)
                                    <x-signal.ui.badge tone="success">{{ $website->health_status }}</x-signal.ui.badge>
                                @elseif ($website->health_status === \App\Modules\Deployer\Models\Website::HEALTH_UNHEALTHY)
                                    <x-signal.ui.badge tone="danger">{{ $website->health_status }}</x-signal.ui.badge>
                                @else
                                    <x-signal.ui.badge>{{ $website->health_status }}</x-signal.ui.badge>
                                @endif
                                @if ($website->health_check_enabled)
    ...
    @else
        <div class="max-w-3xl mx-auto">
            ...
            >
                <x-slot:button>
                    @if (array_filter($filters, fn ($value) => $value !== null))
                        <x-signal.ui.button :href="route('websites.index')" variant="primary">{{ __('Clear filters') }}</x-signal.ui.button>
                    @else
                        <x-signal.ui.button
                            :href="$websiteCreateUrl"
                            data-modal-trigger="website-create-dialog"
