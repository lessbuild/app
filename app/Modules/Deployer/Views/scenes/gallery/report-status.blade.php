<x-layouts.app>
    ...
    <x-signal.ui.local-nav class="mt-6" :label="__('Report status sections')">
    ...
    @if ($unreadUpdate)
        <x-signal.ui.alert class="mt-6 flex flex-wrap items-center justify-between gap-3 p-4" tone="info" role="status">
            <div>
            ...
            <form method="POST" action="{{ route('notifications.read', $unreadUpdate) }}">
                @csrf
                <x-signal.ui.button type="submit" variant="primary">{{ __('Mark update reviewed') }}</x-signal.ui.button>
            </form>
        </x-signal.ui.alert>
    @endif

    <x-signal.ui.panel as="section" id="report-status-overview" class="ui-panel mt-6 scroll-mt-24 p-5 sm:p-6" aria-labelledby="report-status-heading">
        ...
        @else
            <x-signal.ui.alert class="mt-4 p-3" tone="warning">
        ...
        @if ($report->resolved_at && $report->resolution_note)
            <x-signal.ui.alert class="mt-5 p-4" tone="success">
