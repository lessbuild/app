<x-layouts.app>
    ...
    <div class="mt-6 flex flex-wrap items-center gap-2">
        <x-signal.ui.button :href="route('admin.access-requests.index')" :variant="$status ? 'secondary' : 'primary'">{{ __('All') }} ({{ $counts->sum() }})</x-signal.ui.button>
        @foreach (\App\Modules\Deployer\Models\AccessRequest::STATUSES as $item)
            <x-signal.ui.button :href="route('admin.access-requests.index', ['status' => $item])" :variant="$status === $item ? 'primary' : 'secondary'">{{ ucfirst($item) }} ({{ $counts[$item] ?? 0 }})</x-signal.ui.button>
        @endforeach
        <x-signal.ui.button href="{{ route('admin.access-requests.export', array_filter(['status' => $status])) }}" variant="secondary">{{ __('Export CSV') }}</x-signal.ui.button>
    </div>
