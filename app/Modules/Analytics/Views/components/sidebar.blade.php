<div class="flex h-full flex-col gap-7 p-5">
    ...
    <div class="rounded-panel border border-line bg-surface-muted p-3"><p class="text-[0.68rem] font-extrabold uppercase tracking-wider text-subtle">Workspace</p><p class="mt-1 truncate text-sm font-bold text-ink">{{ $workspace?->name ?? 'Your workspace' }}</p><a class="mt-2 inline-block text-xs font-bold text-muted underline hover:text-ink" href="{{ route('analytics.workspaces.index') }}">Switch workspace</a></div>
    ...
    <div class="border-t border-line pt-4"><p class="truncate text-sm font-bold text-ink">{{ auth()->user()->name }}</p><p class="truncate text-xs text-muted">{{ auth()->user()->email }}</p><a class="mt-2 inline-block text-xs font-bold text-muted underline hover:text-ink" href="{{ route('analytics.account.profile') }}">Profile and security</a><p class="mt-3 text-[0.68rem] leading-5 text-subtle">Cookieless estimates · 90-day detail retention</p></div>
</div>
