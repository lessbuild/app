...
class SearchAlertsRequest extends FormRequest
{
    public function authorize(CurrentWorkspace $currentWorkspace): bool
    {
        if ($rule = $this->route('alertRule')) {
            abort_unless($rule instanceof AlertRule && AlertRule::withTrashed()->forWorkspace($currentWorkspace->get())->visibleTo($this->user(), $currentWorkspace->get())->whereKey($rule->id)->exists(), 404);
