...
<div class="space-y-6">
    ...
    @can('create', [\App\Modules\Monitor\Models\Deployment::class, $environment])
    <x-signal.ui.panel as="section" class="p-5"><h2 class="font-bold">Record from your CI pipeline</h2><p class="mt-2 text-xs leading-5 text-muted dark:text-subtle">POST JSON to <code class="wrap-anywhere">{{ route('monitor.api.deployments.store') }}</code> with this environment’s active ingestion token as a Bearer token. Required fields: deployment_id (UUID) and version. Optional: service, service_namespace, commit_sha, note and deployed_at (ISO 8601 with timezone). Match the service labels used by your telemetry.</p><pre class="library-code mt-4"><code>{
  "deployment_id": "85e466e4-9c53-4f2c-a9c8-d987bbfce552",
  "version": "build-42",
  "service": "api",
  "service_namespace": "shop"
}</code></pre><p class="mt-3 text-xs leading-5 text-muted dark:text-subtle">Generate a new UUID per completed deployment, including rollbacks. Reuse the UUID and unchanged details on retry: 201 means created, 200 means replay, 409 means conflicting details. Omitting deployed_at records server time on the first request. Use HTTPS before transmitting production tokens.</p></x-signal.ui.panel>
    @endcan
</div>
@endsection
