...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <section class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(300px,0.7fr)]">
            <x-signal.ui.card as="div" class="px-6 py-6 sm:px-8">
                <div class="relative flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
                    ...
                    <x-signal.ui.card as="div" class="shadow-none shrink-0 bg-surface-muted px-5 py-4 sm:min-w-52">
                        ...
                        <x-signal.ui.progress class="mt-3 bg-line" label="Monthly signal usage" :value="$usagePercentage" />
            ...
            <x-signal.ui.card as="div" class="p-5">
                ...
                @if($corePlanAuthority)
                    <div class="mt-5 border-t border-line pt-4 text-[11px] leading-5 text-muted dark:border-line dark:text-subtle">Core is the entitlement authority for this workspace. Monitor’s Stripe subscription and plan remain separate from Deployer and Analytics.</div>
                    @if($portalAvailable)
                        <form method="POST" action="{{ route('monitor.settings.billing.portal') }}" class="mt-3">
                            @csrf
                            <x-monitor::ui.button variant="quiet" size="sm">Manage Monitor billing in Stripe <x-monitor::icon name="external" class="h-3.5 w-3.5" /></x-monitor::ui.button>
                        </form>
                    @endif
        ...
        @if($usageState === 'warning')
            <x-signal.ui.alert as="section" tone="warning" class="block p-5 sm:p-6"><div class="flex gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-control bg-warning-soft text-warning"><x-monitor::icon name="alert" class="h-4 w-4" /></span><div><h2 class="text-sm font-bold text-warning dark:text-warning">Monthly event usage is at {{ $usagePercentage }}%</h2><p class="mt-1 max-w-2xl text-xs leading-5 text-warning dark:text-warning">You have {{ number_format($remainingEvents) }} events remaining this month. Upgrade now to keep headroom for traffic spikes.</p></div></div></x-signal.ui.alert>
        @elseif($planLimitReached)
            <x-signal.ui.alert as="section" tone="danger" class="block p-5 sm:p-6"><div class="flex gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-control bg-danger-soft text-danger"><x-monitor::icon name="alert" class="h-4 w-4" /></span><div><h2 class="text-sm font-bold text-danger dark:text-danger">Monthly event limit reached</h2><p class="mt-1 max-w-2xl text-xs leading-5 text-danger dark:text-danger">New deliveries stay encrypted and retryable while the workspace is at its allowance. Upgrade the plan, then retry failed deliveries from ingestion diagnostics.</p></div></div></x-signal.ui.alert>
        @elseif($usageState === 'unavailable')
            <x-signal.ui.alert as="section" tone="info" class="border-info/30 bg-info-soft block p-5 sm:p-6"><div class="flex gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-control bg-info-soft text-info"><x-monitor::icon name="alert" class="h-4 w-4" /></span><div><h2 class="text-sm font-bold text-info dark:text-info">Monitor plan could not be verified</h2><p class="mt-1 max-w-2xl text-xs leading-5 text-info dark:text-info">Telemetry intake is paused until this workspace’s subscription and plan snapshot are reconciled in Core.</p></div></div></x-signal.ui.alert>
        @endif
        ...
        <section>
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-4">
                @foreach($plans as $key => $plan)
                    <x-signal.ui.card as="article" class="relative flex flex-col p-5 {{ $planStyles[$key]['border'] }}">
                        ...
                        @elseif($key !== 'free' && in_array($key, $checkoutPlans, true) && ! $hasActiveSubscription)
                            <form method="POST" action="{{ route('monitor.settings.billing.checkout') }}" class="mt-6">
                                @csrf
                                <x-signal.ui.input type="hidden" name="plan" value="{{ $key }}" :restore="false" />
