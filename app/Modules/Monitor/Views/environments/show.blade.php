    ...
    @elseif($canInteract)
        ...
        @if($secret)
            <x-signal.ui.alert as="section" tone="info" class="border-primary/30 bg-primary-soft block p-6">
                ...
                <div class="mt-4 flex flex-col gap-3 sm:flex-row"><x-signal.ui.input id="issued-token" type="text" readonly autocomplete="off" aria-label="New ingestion token" value="{{ $secret }}" class="min-w-0 flex-1 font-mono" :restore="false" /><x-monitor::ui.button type="button" data-copy-target="issued-token"><span data-copy-label>Copy token</span></x-monitor::ui.button></div>
            </x-signal.ui.alert>
        @endif
        <x-signal.ui.panel as="section" data-connection-watch data-connection-url="{{ route('monitor.environments.connection', [$application, $environment]) }}" data-initial-count="{{ $environment->event_count }}" class="p-6">
        ...
        @if($canManage)
            @if(parse_url(config('app.url'), PHP_URL_SCHEME) !== 'https')<x-signal.ui.alert as="p" tone="warning" class="block p-4 text-xs leading-5 text-warning dark:text-warning">This preview uses HTTP. Configure HTTPS before sending production credentials or sensitive telemetry.</x-signal.ui.alert>@endif
