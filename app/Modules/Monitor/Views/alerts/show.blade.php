...
<form method="POST" action="{{ route('monitor.alerts.destinations', $alertRule) }}" class="ui-panel space-y-4 p-6">
    @csrf @method('PUT')<input type="hidden" name="version" value="{{ $alertRule->state_version }}">
...
<form method="POST" action="{{ route('monitor.alerts.escalations', $alertRule) }}" class="ui-panel space-y-4 p-6">
    @csrf @method('PUT')<input type="hidden" name="version" value="{{ $alertRule->state_version }}">
    ...
    @if($escalationLimit === 0)
        <div class="ui-alert border-primary/30 bg-primary-soft block p-4 text-xs leading-5 text-primary dark:text-primary"><p class="font-bold">Escalations are available on paid plans.</p><p class="mt-1">Upgrade to Pro or above to notify additional destinations after a delay.</p><a href="{{ route('monitor.settings.billing') }}" class="mt-2 inline-flex font-bold underline">Review plans →</a></div>
    @else
        ...
        <div class="space-y-4">
            @for($position = 0; $position < min($escalationLimit ?? 10, 10); $position++)
                ...
                <div class="ui-card shadow-none grid gap-4 p-4 sm:grid-cols-[auto_1fr_1fr] sm:items-end">
...
<form method="POST" action="{{ route('monitor.alerts.destroy', $alertRule) }}" data-confirm="Archive this rule and close its active incident? Incident history will be retained." class="ui-panel shadow-none flex flex-col items-start gap-3 p-5">
    @csrf @method('DELETE') <input type="hidden" name="version" value="{{ $alertRule->state_version }}">
