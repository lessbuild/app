<x-signal.ui.panel as="section" class="space-y-5 p-6">
    ...
    @if(!request()->isSecure())<x-signal.ui.alert as="p" tone="warning" class="block p-4 text-sm text-warning dark:text-warning">This preview uses HTTP. Use disposable test keys only. Configure HTTPS before sending production credentials, then rotate keys created during preview.</x-signal.ui.alert>@endif
    ...
    <div class="flex flex-wrap gap-3">
        <form method="POST" action="{{ route('monitor.monitors.heartbeat-key.store', $monitor) }}">@csrf<x-signal.ui.input type="hidden" name="version" value="{{ $monitor->state_version }}" :restore="false" /><x-monitor::ui.button variant="secondary">{{ $monitor->heartbeat_token_hash ? 'Rotate heartbeat key' : 'Generate heartbeat key' }}</x-monitor::ui.button></form>
        @if($monitor->heartbeat_token_hash)<form method="POST" action="{{ route('monitor.monitors.heartbeat-key.destroy', $monitor) }}">@csrf @method('DELETE')<x-signal.ui.input type="hidden" name="version" value="{{ $monitor->state_version }}" :restore="false" /><x-monitor::ui.button variant="secondary">Revoke key and pause</x-monitor::ui.button></form>@endif
    </div>
