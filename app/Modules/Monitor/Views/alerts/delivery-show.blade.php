...
<x-signal.ui.panel as="section" class="grid gap-4 p-6 sm:grid-cols-2">
...
<x-signal.ui.panel as="form" method="POST" action="{{ route('monitor.alert-deliveries.retry', $delivery) }}" class="shadow-none space-y-4 border-warning p-5 dark:border-warning">
    @csrf<x-signal.ui.input type="hidden" name="generation" value="{{ $delivery->generation }}" :restore="false" />
...
<x-signal.ui.card as="div" class="overflow-x-auto"><x-monitor::ui.table caption="Notification delivery attempts" :framed="false">
...
</x-monitor::ui.table></x-signal.ui.card>{{ $attempts->links() }}</section>
@endsection
