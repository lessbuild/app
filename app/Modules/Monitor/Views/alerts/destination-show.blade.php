...
@if($issuedSecret)
@if(parse_url(config('app.url'), PHP_URL_SCHEME) !== 'https')<x-signal.ui.alert as="p" tone="warning" class="block p-4 text-xs leading-5 text-warning dark:text-warning">This preview uses HTTP. Configure HTTPS and rotate this key before using production integrations.</x-signal.ui.alert>@endif
<x-signal.ui.alert as="section" tone="info" class="border-primary/30 bg-primary-soft block space-y-3 p-5"><h2 class="font-bold">Save this signing key now</h2><p class="text-sm">It will not be shown again. Store it securely in your receiver configuration.</p><code class="block break-all rounded-control bg-surface p-3 text-sm dark:bg-surface">{{ $issuedSecret }}</code></x-signal.ui.alert>
@endif
@if($destination->type === \App\Modules\Monitor\Data\Telemetry\AlertDestinationType::Email && ! $mailConfigured)<x-signal.ui.card as="p" class="shadow-none border-warning p-4 text-sm text-warning dark:border-warning dark:text-warning">Email transport is not configured. Set SMTP credentials and ALERT_MAILER=alert_smtp on the server before testing.</x-signal.ui.card>@endif
<x-signal.ui.panel as="section" class="space-y-3 p-5 text-sm">
    ...
    <div class="flex flex-wrap gap-3">
        @if($destination->enabled)<form method="POST" action="{{ route('monitor.alert-destinations.test', $destination) }}">@csrf<x-signal.ui.input type="hidden" name="version" value="{{ $destination->state_version }}" :restore="false" /><x-monitor::ui.button variant="secondary">Send test notification</x-monitor::ui.button></form>@endif
        @if($destination->type === \App\Modules\Monitor\Data\Telemetry\AlertDestinationType::Webhook)<form method="POST" action="{{ route('monitor.alert-destinations.rotate', $destination) }}" data-confirm="Rotate the signing key? Your receiver must be updated. In-flight requests may use the previous key.">@csrf<x-signal.ui.input type="hidden" name="version" value="{{ $destination->state_version }}" :restore="false" /><x-monitor::ui.button variant="secondary">Rotate signing key</x-monitor::ui.button></form>@endif
    </div>
    @endcan
</x-signal.ui.panel>
<section class="space-y-4">
    <h2 class="text-lg font-bold">Delivery history</h2>
    <x-signal.ui.card as="div" class="overflow-x-auto"><x-monitor::ui.table caption="Notification delivery history" :framed="false">
...
@can('delete', $destination)<x-signal.ui.panel as="form" method="POST" action="{{ route('monitor.alert-destinations.destroy', $destination) }}" data-confirm="Archive this destination? Queued deliveries will be cancelled; in-flight requests may finish." class="shadow-none flex flex-col items-start gap-3 p-5">@csrf @method('DELETE')<x-signal.ui.input type="hidden" name="version" value="{{ $destination->state_version }}" :restore="false" /><p class="text-xs text-muted dark:text-subtle">Archive to stop new deliveries and retain history. Requests already in flight may finish.</p><x-monitor::ui.button variant="danger">Archive destination</x-monitor::ui.button></x-signal.ui.panel>@endcan
@endsection
