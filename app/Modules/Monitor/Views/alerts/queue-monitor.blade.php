...
<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <section class="ui-panel p-5"><h2 class="text-xs text-muted dark:text-subtle">{{ $label }}</h2><p class="mt-2 text-3xl font-bold">{{ $snapshot?->{$field} === null ? '—' : number_format($snapshot->{$field}) }}</p><p class="mt-2 text-xs text-muted dark:text-subtle">{{ $current['fresh_snapshot'] ? 'Latest fresh sample' : 'Missing or stale sample' }}</p></section>
    @endforeach
    <section class="ui-panel p-5"><h2 class="text-xs text-muted dark:text-subtle">Live workers · current revision</h2><p class="mt-2 text-3xl font-bold">{{ $current['active_workers'] }}</p><p class="mt-2 text-xs text-muted dark:text-subtle">{{ $current['busy_workers'] }} busy · {{ $current['long_running_workers'] }} overlong</p></section>
</div>
...
<section class="space-y-4">
    ...
    <div class="ui-card overflow-x-auto"><x-monitor::ui.table caption="Worker heartbeat history" :framed="false">
...
<section class="space-y-4">
    ...
    <div class="ui-card overflow-x-auto"><x-monitor::ui.table caption="Queue snapshot history" :framed="false">
...
<section class="space-y-4">
    ...
    <div class="ui-card overflow-x-auto"><x-monitor::ui.table caption="Queue evaluation history" :framed="false">
