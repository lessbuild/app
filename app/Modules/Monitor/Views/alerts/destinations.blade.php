...
@unless($mailConfigured)<x-signal.ui.alert as="p" tone="warning" class="block p-4 text-xs leading-5 text-warning dark:text-warning">Email transport is not configured. Configure your server’s SMTP credentials and set ALERT_MAILER=alert_smtp. Log and array mailers never count as delivery. Slack, Teams, PagerDuty, Discord and HTTPS webhooks can be configured independently.</x-signal.ui.alert>@endunless
...
<div class="grid gap-4 lg:grid-cols-2">
    ...
    <x-signal.ui.card as="a" href="{{ route('monitor.alert-destinations.show', $destination) }}" class="space-y-3 p-5 transition hover:border-primary dark:hover:border-primary">
