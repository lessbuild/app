...
@section('content')
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        @if($environmentOptions === [])
            <p class="ui-card shadow-none p-6 text-sm">Create an application and environment before adding an objective. <a href="{{ route('monitor.applications.index') }}" class="font-semibold text-primary dark:text-primary">Manage applications →</a></p>
        @else
            <form method="POST" action="{{ $objective ? route('monitor.objectives.update', $objective) : route('monitor.objectives.store') }}" class="ui-panel space-y-5 p-6">
                @csrf
                ...
                <div class="ui-card shadow-none p-4"><p class="text-xs font-bold text-ink dark:text-ink">Availability settings</p><p class="mt-1 text-xs leading-5 text-muted dark:text-subtle">A request is healthy when its reported status falls inside this inclusive range. Requests without a status are unknown.</p><div class="mt-4 grid gap-5 sm:grid-cols-2"><x-monitor::ui.input name="status_min" label="Healthy status minimum" type="number" min="100" max="599" :value="$objective?->status_min ?? 200" /><x-monitor::ui.input name="status_max" label="Healthy status maximum" type="number" min="100" max="599" :value="$objective?->status_max ?? 399" /></div></div>
                <div class="ui-card shadow-none p-4"><p class="text-xs font-bold text-ink dark:text-ink">Latency settings</p><p class="mt-1 text-xs leading-5 text-muted dark:text-subtle">Only requests with a non-negative duration are measured. The threshold is inclusive and is ignored for availability objectives.</p><div class="mt-4 max-w-sm"><x-monitor::ui.input name="latency_threshold_ms" label="Healthy duration threshold (ms)" type="number" step="0.001" min="0.001" max="600000" :value="$objective?->latency_threshold_ms" /></div></div>
