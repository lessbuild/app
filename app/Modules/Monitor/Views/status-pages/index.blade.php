        ...
        @forelse($statusPages as $statusPage)
            <article class="ui-card p-5">
                ...
                <div class="mt-5 flex flex-wrap items-center gap-3 border-t border-line pt-4 text-xs dark:border-line">
                    @if($statusPage->published)<a href="{{ route('monitor.status-pages.public', ['statusPage' => $statusPage->slug]) }}" target="_blank" rel="noopener" class="font-bold text-primary hover:underline dark:text-primary">Open public page <x-monitor::icon name="external" class="inline h-3 w-3" /></a>@endif
