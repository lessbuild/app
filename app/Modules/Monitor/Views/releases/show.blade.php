...
<div class="space-y-6">
    ...
    <section class="ui-panel overflow-hidden">
        ...
        <div class="divide-y divide-line dark:divide-line">@forelse($deployments as $deployment)<a href="{{ route('monitor.deployments.show', [$release->application_id, $deployment->environment_id, $deployment->id]) }}" class="flex flex-wrap justify-between gap-3 px-5 py-4 text-sm hover:bg-surface-muted dark:hover:bg-surface-muted"><span class="font-semibold">{{ $deployment->environment->name }}</span><span class="text-xs text-muted dark:text-subtle">{{ $deployment->deployed_at->utc()->format('Y-m-d H:i:s') }} UTC · {{ match ($deployment->source) { 'api' => 'API', 'integration' => 'Deployer integration', default => 'Manual' } }}</span></a>@empty<p class="p-6 text-sm text-muted dark:text-subtle">No deployment recorded for this version. Observing telemetry does not create a deployment.</p>@endforelse</div>
