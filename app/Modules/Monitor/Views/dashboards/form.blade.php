...
<div class="mx-auto max-w-3xl space-y-6">
    ...
    @error('plan')<p class="ui-alert ui-alert-warning block p-4 text-sm text-warning dark:text-warning">{{ $message }}</p>@enderror
    <form method="POST" action="{{ $dashboard->exists ? route('monitor.dashboards.update', $dashboard) : route('monitor.dashboards.store') }}" class="space-y-6">
        ...
        <fieldset class="ui-card space-y-4 p-6" aria-describedby="widgets-help{{ $errors->has('widgets') ? ' widgets-error' : '' }}">
