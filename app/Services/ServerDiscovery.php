...
BASH);
        ...
        foreach (preg_split('/\R/', trim($result->getOutput())) ?: [] as $line) {
            if (preg_match('/\A([a-z0-9_]+)=(.{0,500})\z/', $line, $match)) {
        ...
        if (! in_array($facts['os_version'] ?? '', config('lessbuild.supported_ubuntu_versions', []), true)) {
            throw new RuntimeException('This Ubuntu release is not supported. Use a currently supported LTS release.');
        }
        if (! in_array($facts['architecture'] ?? '', ['x86_64', 'aarch64', 'arm64'], true)) {
            throw new RuntimeException('This server architecture is not supported.');
        }
        ...
        if ((int) ($facts['disk_free_mb'] ?? 0) < 4096) {
            $warnings[] = 'Less than 4 GB of disk space is available.';
        }
        if ((int) ($facts['memory_mb'] ?? 0) < 1024) {
            $warnings[] = 'Less than 1 GB of memory is available.';
        }
