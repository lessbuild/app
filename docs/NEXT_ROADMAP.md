...
controller now delegates to an injected `InfrastructureCostQuery`, catalog
refreshes record `sizes.catalog_synced_at`, and the UI distinguishes catalog
estimates, measured CPU telemetry and unavailable provider billing. The focused
cost/catalog/provider set passed **10 tests / 39 assertions** with strict
warning/deprecation flags. The exact next task is read-only preview
lifetime/quota visibility and direct-versus-shared server/environment
attribution. Review-only cleanup recommendations and provider/cloud acceptance
remain separate.
