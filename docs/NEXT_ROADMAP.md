...
| [Render previews](https://render.com/docs/preview-environments) | PR environments instantiate Blueprint services and datastores, support initialization and automatic cleanup; existing data is not copied | `PreviewDeploymentLifecycle` persists a template-driven Laravel queue/scheduler/PostgreSQL/Valkey stack manifest, and signed deployment callbacks now record planned/provisioning/ready/failed local resource status. Remote cleanup, ownership, quotas and independent provider-readiness verification still need completion before claiming full-stack preview parity. |
...
3. **Complete preview environments.** Phases 3A and 3B now add the local, template-driven Laravel queue/scheduler/PostgreSQL/Valkey stack manifest and callback-backed resource readiness states. Continue with explicit initialization secrets, atomic quotas, ownership-aware expiry and retryable stale-attempt-safe cleanup. Completion: open/update/close/expiry cycles work for a multi-service fixture and leave no orphaned resources.
