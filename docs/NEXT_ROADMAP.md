...
are revoked before any start. The local installer/dry-run contract is tested,
but no host has been installed or used for remote cleanup proof. The exact next
task is authorized installed-host verification of normal and abnormal cleanup,
...
network-partition evidence. A separate
...
local adapter boundary only and does not replace installed-host, network-
partition or uncooperative-remote evidence.
