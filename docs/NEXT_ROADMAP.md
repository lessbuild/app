...
The configuration-specific provider acceptance sequence is complete on the
isolated dev runtime. It covered plan/review/apply, real delivery,
idempotency, secret freshness, workspace ownership, approval/cancellation,
explicit retry, reversible health failure, local environment removal and
disposable-resource cleanup. See
[the verification record](verification/configuration-acceptance-2026-09-17.md).
The next task is provider-backed preview-stack acceptance for managed
PostgreSQL/Valkey readiness and cleanup. This is separate from the earlier
generic deployment/rollback/backup/restore drill and from production/live
acceptance.
