...
9. **Resource usage and cost visibility — source and preview context complete locally.** The cost page now exposes an organization-scoped cost read model, records provider-catalog observation timestamps, distinguishes catalog estimates from measured CPU telemetry and states that provider billing is not connected. Unknown prices remain excluded from totals. It also shows exact active-preview quota usage and bounded configured lifetimes/expiry projections without mutating lifecycle state or treating quota as money. Next add direct-versus-shared server/environment attribution and review-only cleanup recommendations for explicitly owned temporary resources.
...
272 assertions** with strict warning/deprecation flags. The exact next task is
direct-versus-shared server/environment attribution and review-only cleanup
signals for explicitly owned temporary resources. Provider billing, automatic
