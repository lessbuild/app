...
| [Render previews](https://render.com/docs/preview-environments) | PR environments instantiate Blueprint services and datastores, support initialization and automatic cleanup; existing data is not copied | `PreviewDeploymentLifecycle` now persists a template-driven Laravel queue/scheduler/PostgreSQL/Valkey stack manifest. Remote initialization, readiness and cleanup still need verification before claiming full-stack preview parity. |
...
3. **Complete preview environments.** Phase 3A has added the local, template-driven Laravel queue/scheduler/PostgreSQL/Valkey stack manifest. Continue with remote initialization, readiness, explicit initialization secrets, fork handling, atomic quotas and retryable cleanup. Completion: open/update/close/expiry cycles work for a multi-service fixture and leave no orphaned resources.
