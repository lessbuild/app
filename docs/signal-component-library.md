...
The upstream Signal source is `https://github.com/lessbuild/template` on `main`. The latest source checked for this integration is `594c17b511533339bb80a2d17b7b2314c0496602` (`Record dashboard release verification`, 2026-09-24), including the dashboard widget-layout work from `1cc22eb`. Since `cade4159eb13b8609e69c1b7728a69d67c04f22c`, the theme tokens and shared component source are unchanged; the newer dashboard example adds keyboard-accessible widget ordering and browser-saved date and segment filters. The Laravel page-header component follows the current responsive Signal composition and retains application-specific icon, metadata, and action slots. App theme tokens match the upstream theme file; the component stylesheet also retains Laravel-specific accessibility, topbar, and code-block adaptations. Signal adds a full record-detail composition and improves responsive custom-popover positioning and side-sheet focus, dismissal, and scroll-lock behavior. This application currently uses native disclosure menus rather than Signal's `data-popover-toggle` behavior. The side-sheet behavior is adapted into `x-signal.overlays.side-sheet` and `x-signal.overlays.side-sheet-trigger`; Deployer's project view uses them to navigate its environments. The record-detail composition maps to the live project-detail screens, which use authorized product data rather than static sample records. The product drawer begins below the live topbar height so Deployer’s workspace and project context remains visible; its height follows the responsive topbar as content wraps. Deployer already stores dashboard widget visibility in the user profile; template-only browser storage and sample metrics are not used for product data.
