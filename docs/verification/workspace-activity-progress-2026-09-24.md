...
## 25 September 2026 — project mapped website DNS/TLS status (verification deferred)

The Deployer activity feed now shows DNS and TLS status for website domains only when their websites resolve through the existing resource-map service to one active Core project and Deployer organization. Active domains are omitted; DNS/TLS errors, expiring certificates, manual DNS setup, and stale pending checks appear with separate status steps and an authorized website link. The projection selects no hostname, DNS record/provider identifier, certificate secret, or error text. A deferred regression covers active, failed, blocked/expiring, manual setup, stale, cross-organization, and healthy states with result-link and redaction checks. It has not been run under the plan-wide test deferral.
