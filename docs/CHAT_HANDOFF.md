...
hostnames now return permanent redirects to the new canonical host.

The public DNS A record is active at the authoritative DNS provider (host
`deployer`, value `174.138.39.41`). Caddy obtained a valid Let’s Encrypt
...
- Desktop bounded route sweep at 1440px: **68 pages passed**, with zero HTTP,
  layout or browser errors.
- Canonical-host Livewire/mobile smoke: **1 Playwright test passed in 20.8
  seconds** against `https://deployer.buildpusher.com`.
...
that preceded the navigation consolidation. Mobile now shows separate
Applications, Deployments, Repositories, Recipes, Gallery, Billing, Costs,
Account and Settings destinations, including the original active routes. The
desktop sidebar keeps the consolidated Applications, Template library, Billing
and usage, and Account and security entries.

The implementation is pushed as `e81816e` on `origin/main`; the matching
browser expectation fix is `9f381f0`. Both are deployed to the isolated dev
runtime. Focused PHP coverage passed **39 tests / 608 assertions**, Pint and
diff checks passed, and the authenticated dev-domain browser journey passed
on mobile, tablet and desktop (**3 tests**). The persistent
`buildpusher-dev-main.service` is active and enabled.
...
Metric alert, saved-investigation and operational-incident note forms now
render through reusable observability scene dialog components. Existing
resource identity, policy and entitlement checks, URL state, validation keys,
bounded inputs, secret-safe old input, notification behavior and responses
remain unchanged.

Verification: 45 focused PHP tests / 353 assertions and two targeted mobile
browser journeys passed. Blade cache, Pint and diff checks passed.
Implementation commit and push: `7382176`.

Next task: convert alert-destination, status-page and status-incident
create/update forms into URL-backed reusable dialogs, then continue with
...
Alert-destination, status-page, status-update and per-incident review forms
now render through reusable observability scene dialog components. Existing
organization authorization, entitlements, resource scoping, encrypted
destination handling, incident validation, subscriber notifications, field
names and responses remain unchanged.

Verification: 29 focused PHP tests / 262 assertions, one fixture / 140
assertions and one targeted four-workflow mobile browser journey passed. Blade
cache, Pint and diff checks passed. Implementation commit and push:
`4e18d41`.
...
aaa8861 moves application preview settings into a reusable URL-backed dialog.
It preserves the existing preview Form Request and Action,
...
mobile browser, Blade, Pint and diff checks pass.
...
ec85060 moves repository webhook enable/rotate/disable controls into a
repository-context dialog. Existing policy ordering, GitLab token validation,
generated one-time secret flashes, confirmations and canonical redirects
remain unchanged; modal-originated enable/rotate redirects preserve the
