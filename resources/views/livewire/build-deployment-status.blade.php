<div @if ($shouldPoll) wire:poll.5s @endif>
    ...
    @if ($build->risk_assessment)
        <section class="ui-panel mt-4 p-4">
            ...
            <ul class="mt-4 grid gap-2 sm:grid-cols-2">@foreach($build->risk_assessment['checks'] ?? [] as $check)<li class="flex gap-2 rounded-lg border border-line bg-surface-muted p-3 text-sm"><span class="font-black {{ $check['status'] === 'passed' ? 'text-green-600' : ($check['status'] === 'warning' ? 'text-amber-600' : 'text-red-600') }}">{{ $check['status'] === 'passed' ? '✓' : '!' }}</span><span><strong class="block text-ink">{{ $check['name'] }}</strong><span class="text-xs text-muted">{{ $check['detail'] }}</span></span></li>@endforeach</ul>
        </section>
    @endif
