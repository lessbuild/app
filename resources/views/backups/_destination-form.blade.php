...
<form method="POST" action="{{ $action }}" class="mt-4 grid gap-3 sm:grid-cols-2">
    @csrf
    @if($isEdit)
        @method('PATCH')
    @endif
    <div class="sm:col-span-2">
        ...
        <select id="{{ $formId }}-storage_provider" name="storage_provider" class="input secondary mt-1 w-full rounded-sm">
    ...
    <div>
        ...
        <input id="{{ $formId }}-name" name="name" value="{{ old('name', $isEdit ? $destination->name : '') }}" placeholder="{{ __('Offsite backups') }}" class="input secondary mt-1 w-full rounded-sm" required>
    </div>
    <div>
        <label for="{{ $formId }}-region" class="block text-xs font-semibold uppercase text-secondary">{{ __('Region') }}</label>
        <input id="{{ $formId }}-region" name="region" value="{{ old('region', $isEdit ? $destination->region : '') }}" placeholder="{{ __('lon1') }}" class="input secondary mt-1 w-full rounded-sm" required>
    ...
    <div class="sm:col-span-2">
        ...
        <input id="{{ $formId }}-endpoint" type="url" name="endpoint" value="{{ old('endpoint', $isEdit ? $destination->endpoint : '') }}" placeholder="{{ $selectedPreset->endpointHint }}" class="input secondary mt-1 w-full rounded-sm">
    ...
    <div>
        ...
        <input id="{{ $formId }}-bucket" name="bucket" value="{{ old('bucket', $isEdit ? $destination->bucket : '') }}" placeholder="{{ __('buildpusher-backups') }}" class="input secondary mt-1 w-full rounded-sm" required>
    </div>
    <div>
        <label for="{{ $formId }}-path_prefix" class="block text-xs font-semibold uppercase text-secondary">{{ __('Folder prefix') }}</label>
        <input id="{{ $formId }}-path_prefix" name="path_prefix" value="{{ old('path_prefix', $isEdit ? $destination->path_prefix : 'buildpusher') }}" placeholder="{{ __('buildpusher') }}" class="input secondary mt-1 w-full rounded-sm" required>
    </div>
    <div>
        ...
        <input id="{{ $formId }}-access_key" name="access_key" value="" placeholder="{{ $isEdit ? __('Leave blank to keep current key') : __('Access key') }}" autocomplete="off" class="input secondary mt-1 w-full rounded-sm" @required(!$isEdit)>
    </div>
    <div>
        ...
        <input id="{{ $formId }}-secret_key" type="password" name="secret_key" value="" placeholder="{{ $isEdit ? __('Leave blank to keep current secret') : __('Secret key') }}" autocomplete="new-password" class="input secondary mt-1 w-full rounded-sm" @required(!$isEdit)>
    </div>
    ...
    <button type="submit" class="button primary sm:col-span-2">{{ $submitLabel }}</button>
</form>
