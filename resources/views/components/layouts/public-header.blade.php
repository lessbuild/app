...
<div
    ...
    @keydown.escape.window="if (navigationOpen) { navigationOpen = false; $nextTick(() => $refs.mobileNavigationToggle.focus()) }"
    @resize.window="if (window.innerWidth >= 768 && navigationOpen) { navigationOpen = false; $nextTick(() => $refs.desktopNavigation.querySelector('a')?.focus()) }"
>
<header class="relative z-40 border-b border-line bg-surface/90 backdrop-blur">
    <div class="mx-auto flex h-16 max-w-content items-center justify-between gap-6 px-5 sm:px-8">
        ...
        <nav x-ref="desktopNavigation" class="hidden items-center gap-1 md:flex" aria-label="{{ __($navigationLabel) }}">
        ...
        <div class="flex items-center gap-2">
            ...
            <button id="{{ $mobileToggleId }}" x-ref="mobileNavigationToggle" type="button" class="ui-icon-btn md:hidden" aria-controls="{{ $mobileNavigationId }}" :aria-expanded="navigationOpen.toString()" aria-label="{{ __('Open navigation') }}" @click="navigationOpen = true; $nextTick(() => $refs.mobileNavigationClose.focus())">
...
<div id="{{ $mobileNavigationId }}" x-cloak x-show="navigationOpen" x-trap.inert.noscroll="navigationOpen" class="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true" aria-labelledby="{{ $mobileNavigationId }}-title" :aria-hidden="(! navigationOpen).toString()">
    <button type="button" class="absolute inset-0 bg-slate-950/50" aria-label="{{ __('Close navigation') }}" @click="navigationOpen = false; $nextTick(() => $refs.mobileNavigationToggle.focus())"></button>
    <div class="relative ml-auto flex h-full w-[min(21rem,88vw)] flex-col overflow-y-auto border-l border-line bg-surface px-5 py-5 shadow-2xl">
        <div class="flex items-center justify-between">
            ...
            <button type="button" x-ref="mobileNavigationClose" class="ui-icon-btn" aria-label="{{ __('Close navigation') }}" @click="navigationOpen = false; $nextTick(() => $refs.mobileNavigationToggle.focus())">
                <svg class="h-5 w-5 stroke-2" aria-hidden="true"><use xlink:href="/assets/images/icons.svg#close"></use></svg>
            </button>
        </div>
        <nav class="mt-8 flex flex-col gap-1" aria-label="{{ __($mobileNavigationLabel) }}" @click="if ($event.target.closest('a')) { navigationOpen = false; $nextTick(() => $refs.mobileNavigationToggle.focus()) }">
            @foreach ($publicLinks as $link)
