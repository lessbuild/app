...
>
    <form method="POST" action="{{ route('domains.temporary') }}" class="space-y-4">
        ...
        <label class="block" for="temporary-domain-website">
            <span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Website') }}</span>
            <select id="temporary-domain-website" name="website_id" class="input secondary w-full rounded-md" required>
                @foreach ($websites as $website)
                    <option value="{{ $website->id }}" @selected((string) old('website_id') === (string) $website->id)>{{ $website->name }}</option>
                @endforeach
            </select>
        </label>
        <label class="block" for="temporary-domain-dns-provider">
            <span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('DNS provider') }}</span>
            <select id="temporary-domain-dns-provider" name="dns_provider_id" class="input secondary w-full rounded-md" required>
