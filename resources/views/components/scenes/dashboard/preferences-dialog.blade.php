        ...
        <fieldset>
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                @foreach(['stats' => __('Resource totals'), 'setup' => __('Setup progress'), 'status' => __('Platform status'), 'providers' => __('Provider health')] as $widget => $label)
                    <label class="flex items-center gap-2 text-sm text-secondary"><input type="checkbox" name="widgets[]" value="{{ $widget }}" @checked(in_array($widget, $widgets, true))><span>{{ $label }}</span></label>
                @endforeach
            </div>
        </fieldset>
