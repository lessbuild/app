...
>
    <form method="POST" action="{{ route('backups.schedules.store') }}" class="grid gap-4 sm:grid-cols-2">
        ...
        <div>
            ...
            <select id="backup-schedule-website" name="website_id" class="ui-input">
        ...
        <div>
            ...
            <select id="backup-schedule-destination" name="backup_destination_id" class="ui-input">
        ...
        <div>
            ...
            <select id="backup-schedule-frequency" name="frequency" class="ui-input">
        ...
        <div>
            ...
            <input id="backup-schedule-time" type="time" name="run_at" value="{{ old('run_at', '02:00') }}" class="ui-input">
            <x-forms.errors name="run_at" />
        </div>
        <div>
            <label class="ui-label" for="backup-schedule-weekday">{{ __('Weekday') }}</label>
            <input id="backup-schedule-weekday" type="number" name="weekday" min="0" max="6" value="{{ old('weekday', 0) }}" class="ui-input" title="{{ __('Sunday is 0') }}">
            <x-forms.errors name="weekday" />
        </div>
        <div>
            <label class="ui-label" for="backup-schedule-retention">{{ __('Retention count') }}</label>
            <input id="backup-schedule-retention" type="number" name="retention_count" min="1" max="365" value="{{ old('retention_count', 14) }}" class="ui-input">
