    ...
    <main id="main-content" class="min-h-screen bg-secondary px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-5xl">
            ...
            <div class="mt-10 grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
                ...
                <section class="rounded-2xl border border-primary bg-primary p-6 shadow-sm sm:p-8">
                    ...
                    @else
                        ...
                        <form method="POST" action="{{ route('access-request.store') }}" class="mt-6 grid gap-5 sm:grid-cols-2">@csrf
                            <label><span class="mb-1 block text-sm font-semibold text-primary">{{ __('Name') }} *</span><input name="name" value="{{ old('name') }}" required autocomplete="name" class="input secondary rounded"></label>
                            <label><span class="mb-1 block text-sm font-semibold text-primary">{{ __('Work email') }} *</span><input type="email" name="email" value="{{ old('email') }}" required autocomplete="email" class="input secondary rounded"></label>
                            <label><span class="mb-1 block text-sm font-semibold text-primary">{{ __('Company or project') }}</span><input name="company" value="{{ old('company') }}" autocomplete="organization" class="input secondary rounded"></label>
                            <label><span class="mb-1 block text-sm font-semibold text-primary">{{ __('Team size') }}</span><select name="team_size" class="input secondary rounded"><option value="">{{ __('Select') }}</option>@foreach(\App\Models\AccessRequest::TEAM_SIZES as $size)<option value="{{ $size }}" @selected(old('team_size')===$size)>{{ $size }}</option>@endforeach</select></label>
                            <label class="sm:col-span-2"><span class="mb-1 block text-sm font-semibold text-primary">{{ __('Plan of interest') }}</span><select name="plan" class="input secondary rounded"><option value="">{{ __('Not sure yet') }}</option>@foreach(config('billing.plans') as $key=>$plan)<option value="{{ $key }}" @selected(old('plan',$selectedPlan)===$key)>{{ $plan['name'] }}</option>@endforeach</select></label>
                            <label class="sm:col-span-2"><span class="mb-1 block text-sm font-semibold text-primary">{{ __('What do you want to deploy?') }} *</span><textarea name="use_case" required minlength="20" maxlength="2000" rows="6" class="input secondary rounded" placeholder="{{ __('Current stack, provider, number of servers, and the problem you want BuildPusher to solve.') }}">{{ old('use_case') }}</textarea><span class="mt-1 block text-xs text-secondary">{{ __('Please do not include passwords, tokens, or other secrets.') }}</span></label>
