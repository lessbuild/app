...
<section class="ui-card mt-8 p-6" id="operational-incidents">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <p class="text-xs font-bold uppercase tracking-widest text-ternary">{{ __('Private response') }}</p>
            <div class="mt-1 flex flex-wrap items-center gap-3">
                <h2 class="text-xl font-black text-primary">{{ __('Operational incidents') }}</h2>
            ...
            <p class="mt-1 max-w-3xl text-sm leading-6 text-secondary">{{ __('Failures are grouped by resource, assigned to a responder, and closed automatically when recovery is detected.') }}</p>
        </div>
    ...
    <div id="operational-incident-active" class="mt-5 space-y-3">
        ...
        @empty
            @if ($resolvedOperationalIncidents->isEmpty())
                ...
                />
            @else
                <p class="rounded-lg border border-primary bg-secondary p-4 text-sm text-secondary">{{ __('No active operational incidents.') }}</p>
            @endif
        @endforelse
    </div>

    @if ($resolvedOperationalIncidents->isNotEmpty())
        <details id="operational-incident-history" class="mt-5 rounded-xl border border-primary bg-primary p-4">
