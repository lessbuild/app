    ...
    <div class="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        @if($canManage)
            <aside class="space-y-5">
                <form method="POST" action="{{ route('domains.store') }}" class="rounded-2xl border border-primary bg-primary p-5">@csrf<h2 class="font-black text-primary">{{ __('Add domain') }}</h2><div class="mt-4 space-y-3"><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Website') }}</span><select name="website_id" class="input secondary rounded-sm" required>@foreach($websites as $website)<option value="{{ $website->id }}">{{ $website->name }}</option>@endforeach</select></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Hostname') }}</span><input name="hostname" placeholder="www.example.com" class="input secondary rounded-sm" required></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Behavior') }}</span><select name="type" class="input secondary rounded-sm"><option value="alias">{{ __('Serve application') }}</option><option value="redirect">{{ __('Redirect') }}</option></select></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Redirect destination') }}</span><input type="url" name="redirect_url" placeholder="https://example.com" class="input secondary rounded-sm"></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('DNS automation') }}</span><select name="dns_provider_id" class="input secondary rounded-sm"><option value="">{{ __('Manual DNS') }}</option>@foreach($dnsProviders as $provider)<option value="{{ $provider->id }}">{{ $provider->name }}</option>@endforeach</select></label><button type="submit" class="button primary w-full">{{ __('Add domain') }}</button></div></form>
                <form method="POST" action="{{ route('domains.temporary') }}" class="rounded-2xl border border-primary bg-primary p-5">@csrf<h2 class="font-black text-primary">{{ __('Temporary domain') }}</h2><p class="mt-1 text-sm text-secondary">{{ $temporaryBaseDomain ? __('Issue a shareable :domain address.', ['domain'=>'*.'.$temporaryBaseDomain]) : __('Set TEMPORARY_APP_DOMAIN to enable this feature.') }}</p><div class="mt-4 space-y-3"><select name="website_id" class="input secondary rounded-sm" required>@foreach($websites as $website)<option value="{{ $website->id }}">{{ $website->name }}</option>@endforeach</select><select name="dns_provider_id" class="input secondary rounded-sm" required><option value="">{{ __('Select Cloudflare provider') }}</option>@foreach($dnsProviders as $provider)<option value="{{ $provider->id }}">{{ $provider->name }}</option>@endforeach</select><button type="submit" class="button primary w-full" @disabled(!$temporaryBaseDomain)>{{ __('Issue domain') }}</button></div></form>
            </aside>
        @endif
    </div>
</x-layouts.app>
