...
>
    ...
    <nav aria-label="{{ __('Project status') }}" class="mb-6 flex flex-wrap gap-2">
        <x-signal.ui.button
            :variant="$projectStatus === 'active' ? 'primary' : 'secondary'"
            :href="route('core.projects.index', $workspace)"
            :aria-current="$projectStatus === 'active' ? 'page' : null"
        >{{ __('Active projects') }}</x-signal.ui.button>
        <x-signal.ui.button
            :variant="$projectStatus === 'archived' ? 'primary' : 'secondary'"
            :href="route('core.projects.index', ['workspace' => $workspace, 'status' => 'archived'])"
            :aria-current="$projectStatus === 'archived' ? 'page' : null"
        >{{ __('Archived') }}</x-signal.ui.button>
    </nav>
    ...
    @if ($projects->isEmpty())
        <x-signal.ui.empty-state
            :title="$projectStatus === 'archived' ? __('No archived projects') : __('No projects yet')"
            :description="$projectStatus === 'archived' ? __('Projects you archive appear here. Their resource mappings and connection history stay preserved.') : __('Create a project once, then connect the product modules your team needs.')"
            icon="view-grid"
        >
            @if ($projectStatus === 'active' && $canCreateProjects)
                <x-slot:action>
                    <x-signal.ui.button variant="primary" :href="route('core.projects.create', $workspace)">
            ...
            @elseif ($projectStatus === 'archived')
                <x-slot:action>
                    <p class="text-sm text-muted">{{ __('There are no archived projects to restore.') }}</p>
                </x-slot:action>
            @else
                <x-slot:action>
            ...
            @foreach ($projects as $project)
                <x-signal.ui.card tone="interactive" class="p-5 sm:p-6">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="min-w-0">
                            <p class="ui-eyebrow">{{ $projectStatus === 'archived' ? __('Archived project') : __('Project') }}</p>
                        ...
                        <x-signal.ui.badge :tone="$projectStatus === 'archived' ? 'neutral' : 'success'">{{ $projectStatus === 'archived' ? __('Archived') : __('Active') }}</x-signal.ui.badge>
                    </div>
                    ...
                    <div class="mt-5 grid gap-2 sm:grid-cols-3">
                        @foreach ($products as $key => $label)
                            ...
                            <div class="flex min-w-0 items-center justify-between gap-2 rounded-control border border-line bg-surface-muted px-3 py-2">
                                ...
                                @elseif ($isConnected && $projectStatus === 'active')
                                    <x-signal.ui.badge tone="success">{{ __('Connected') }}</x-signal.ui.badge>
                                @elseif ($isConnected)
                                    <x-signal.ui.badge tone="neutral">{{ __('Linked') }}</x-signal.ui.badge>
                                @else
                    ...
                    <div class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-line pt-4 text-xs text-muted">
                        ...
                        @elseif ($canManageProjects)
                            <form method="POST" action="{{ route('core.projects.restore', [$workspace, $project]) }}">
                                @csrf
                                <x-signal.ui.button variant="primary" type="submit" class="min-h-9 px-3 text-xs">
                                    {{ __('Restore project') }}
                                </x-signal.ui.button>
                            </form>
                        @else
                            <span>{{ __('Only workspace owners and admins can restore projects.') }}</span>
                        @endif
                    </div>
                </x-signal.ui.card>
            @endforeach
