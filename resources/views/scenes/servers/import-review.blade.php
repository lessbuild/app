    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,.8fr)]">
        <div class="space-y-6">
            <section class="ui-card p-5 sm:p-6" aria-labelledby="server-import-discovery-heading">
                ...
                <h2 id="server-import-discovery-heading" class="mt-2 text-xl font-black text-primary">{{ $report['hostname'] ?? __('Unknown host') }}</h2>
                <dl class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ([
                        ...
                        <div>
                            ...
                            <dd class="mt-1 break-words font-semibold text-primary">{{ $value }}</dd>
                        </div>
                    @endforeach
                </dl>
                <div class="mt-6 border-t border-primary pt-5">
                    <h3 class="text-sm font-bold text-primary">{{ __('Detected services') }}</h3>
                    <p class="mt-1 text-sm text-secondary">{{ ($report['services'] ?? []) !== [] ? implode(', ', $report['services']) : __('No managed service binaries detected.') }}</p>
                </div>
            </section>
        ...
        <section class="ui-card h-fit p-5 sm:p-6" aria-labelledby="server-import-approve-heading">
            <p class="text-xs font-bold uppercase tracking-widest text-ternary">{{ __('Explicit approval') }}</p>
            <h2 id="server-import-approve-heading" class="mt-1 text-xl font-black text-primary">{{ __('Approve takeover') }}</h2>
            <p class="mt-2 text-sm leading-6 text-secondary">{{ __('This inspection expires :time. If anything changed after inspection, go back and inspect again.', ['time' => $assessment->expires_at->diffForHumans()]) }}</p>
            ...
            <form method="POST" action="{{ route('servers.import.confirm', $assessment) }}" class="mt-6 space-y-5">
                ...
                <label class="flex items-start gap-3">
                    <input type="checkbox" name="host_fingerprint_confirmed" value="1" required class="mt-1 rounded border-primary bg-primary text-ternary">
                    <span class="text-sm text-secondary">{{ __('I verified the SSH fingerprint through a trusted source.') }}</span>
                </label>
                <label class="flex items-start gap-3">
                    <input type="checkbox" name="backup_confirmed" value="1" required class="mt-1 rounded border-primary bg-primary text-ternary">
                    <span class="text-sm text-secondary">{{ __('I have a current backup or disposable server snapshot and understand existing configuration may change.') }}</span>
                </label>
                <div>
                    <label for="confirmation" class="block text-sm font-bold text-primary">{{ __('Type :name to approve', ['name' => $assessment->configuration['name']]) }}</label>
                    <input id="confirmation" name="confirmation" required autocomplete="off" class="input secondary mt-2 w-full rounded-lg">
                </div>
