    ...
    <div class="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        @foreach ($plans as $key => $plan)
            ...
            <article @class(['relative flex flex-col rounded-2xl border bg-primary p-6 shadow-xs', 'border-ternary ring-2 ring-ternary' => $key === 'pro', 'border-primary' => $key !== 'pro'])>
                ...
                <ul class="my-6 flex-1 space-y-2 text-sm text-secondary">@foreach($plan['features'] as $feature)<li class="flex gap-2"><span class="font-bold text-ternary">✓</span>{{ $feature }}</li>@endforeach<li class="flex gap-2"><span class="font-bold text-ternary">✓</span>{{ number_format($plan['limits']['api_requests_per_minute']) }} {{ __('API requests per minute') }}</li></ul>
