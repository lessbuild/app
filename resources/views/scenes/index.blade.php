...
>
    ...
    <main id="main-content" tabindex="-1">
            ...
            <div class="relative mx-auto grid max-w-content items-center gap-8 px-5 py-12 sm:gap-12 sm:px-8 sm:py-20 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16 lg:py-24">
                ...
                <div data-illustrative-preview class="ui-panel relative overflow-hidden p-3 sm:p-4" aria-label="{{ __('Illustrative workspace preview') }}">
                    ...
                    <div class="grid gap-3 p-2 pt-4 sm:grid-cols-[1.35fr_.65fr]">
                        ...
                        <div class="grid grid-cols-3 gap-3 sm:grid-cols-1">@foreach ([['3', __('Servers')], ['8', __('Websites')], ['0', __('Incidents')]] as [$value, $label])<div class="rounded-card border border-line bg-surface p-4"><p class="text-2xl font-black text-ink">{{ $value }}</p><p class="mt-1 text-xs text-muted">{{ $label }}</p></div>@endforeach</div>
                    </div>
                ...
                <div class="mt-10 overflow-hidden rounded-panel border border-line bg-surface-muted shadow-panel">
                    <div class="overflow-x-auto border-b border-line bg-surface p-2 lg:hidden" aria-label="{{ __('Feature categories') }}">
                        <div class="flex w-max gap-2">
                            @foreach ($featureGroups as $key => $group)
                                <button type="button" class="whitespace-nowrap rounded-card px-4 py-2.5 text-sm font-bold transition" :class="activeFeature === '{{ $key }}' ? 'bg-emphasis text-emphasis-ink shadow-sm' : 'text-muted hover:bg-surface-muted hover:text-ink'" :aria-pressed="(activeFeature === '{{ $key }}').toString()" @click="activeFeature = '{{ $key }}'">{{ $group['label'] }}</button>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="grid lg:grid-cols-[15rem_1fr]">
                        <div class="hidden border-r border-line bg-surface p-3 lg:block" aria-label="{{ __('Feature categories') }}">
                            @foreach ($featureGroups as $key => $group)
                                <button type="button" class="mb-1 flex w-full items-center gap-3 rounded-card px-3 py-3 text-left text-sm font-bold transition last:mb-0" :class="activeFeature === '{{ $key }}' ? 'bg-emphasis text-emphasis-ink shadow-sm' : 'text-muted hover:bg-surface-muted hover:text-ink'" :aria-pressed="(activeFeature === '{{ $key }}').toString()" @click="activeFeature = '{{ $key }}'">
        ...
        <section id="questions" class="scroll-mt-20 border-t border-line bg-surface-muted/60 py-16 sm:py-24">
            <div class="mx-auto grid max-w-content gap-8 px-5 sm:px-8 lg:grid-cols-[.7fr_1.3fr]"><div><p class="ui-eyebrow">{{ __('Good to know') }}</p><h2 class="mt-3 text-3xl font-extrabold tracking-[-0.04em] text-ink">{{ __('Straight answers.') }}</h2></div><div class="space-y-3">@foreach ([[__('Who is :app for?', ['app' => config('app.name')]), __('Development teams and operators who want a focused control plane while keeping applications in their own cloud accounts.')], [__('Where does my application run?'), __('On infrastructure in the provider account you connect. :app coordinates the operational workflow.', ['app' => config('app.name')])], [__('Can I recover a previous release?'), __('Yes. Recorded revisions can be redeployed with lineage and logs retained, provided the target is ready.')], [__('How are sensitive values handled?'), __('Provider tokens, environments, scripts, command text, and retained output are encrypted at rest and owner-scoped.')], [__('What does :app not replace?', ['app' => config('app.name')]), __('Your cloud provider, source host, application architecture, and independent external monitoring remain separate. Provider invoices remain authoritative for cost.')]] as [$question, $answer])<details class="group rounded-card border border-line bg-surface p-5"><summary class="flex cursor-pointer list-none items-center justify-between gap-4 font-bold text-ink">{{ $question }}<span class="text-xl text-ink transition group-open:rotate-45" aria-hidden="true">+</span></summary><p class="mt-3 leading-7 text-muted">{{ $answer }}</p></details>@endforeach</div></div>
        </section>

        <section class="border-t border-line bg-surface py-16 sm:py-24"><div class="mx-auto max-w-content px-5 sm:px-8"><div class="ui-emphasis relative overflow-hidden rounded-panel px-6 py-12 sm:px-12 sm:py-16"><div class="absolute -right-20 -top-24 h-72 w-72 rounded-full bg-primary/30 blur-3xl"></div><div class="relative flex flex-col justify-between gap-8 lg:flex-row lg:items-center"><div><p class="text-xs font-extrabold uppercase tracking-[0.15em] text-brand-300">{{ __('Start with a clear release') }}</p><h2 class="mt-4 max-w-2xl text-3xl font-extrabold tracking-[-0.04em] text-emphasis-ink sm:text-4xl">{{ __('Make the next deployment the clear one.') }}</h2><p class="ui-emphasis-muted mt-4 max-w-xl text-base leading-7">{{ __('Bring infrastructure, releases, and recovery into one workspace.') }}</p></div><a href="{{ $primaryUrl }}" class="ui-btn ui-btn-primary">{{ $primaryLabel }}</a></div></div></div></section>
    </main>
