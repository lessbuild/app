                ...
                <div class="mt-8 overflow-hidden rounded-2xl border border-primary bg-secondary shadow-xl">
                    <div class="overflow-x-auto border-b border-primary bg-primary p-2 lg:hidden" aria-label="{{ __('Feature categories') }}">
                        <div class="flex w-max gap-2">
                            @foreach ($featureGroups as $key => $group)
                                <button type="button" class="whitespace-nowrap rounded-lg px-4 py-2.5 text-sm font-bold transition" :class="activeFeature === '{{ $key }}' ? 'bg-ternary text-white shadow' : 'text-secondary hover:bg-secondary hover:text-primary'" :aria-pressed="(activeFeature === '{{ $key }}').toString()" @click="activeFeature = '{{ $key }}'">{{ $group['label'] }}</button>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="grid lg:grid-cols-[15rem_1fr]">
                        <div class="hidden border-r border-primary bg-primary p-3 lg:block" aria-label="{{ __('Feature categories') }}">
                            @foreach ($featureGroups as $key => $group)
                                <button type="button" class="mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold transition last:mb-0" :class="activeFeature === '{{ $key }}' ? 'bg-ternary text-white shadow' : 'text-secondary hover:bg-secondary hover:text-primary'" :aria-pressed="(activeFeature === '{{ $key }}').toString()" @click="activeFeature = '{{ $key }}'">
