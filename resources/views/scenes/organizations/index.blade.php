        ...
        <x-forms.section :title="__('Members')" :description="__('People with access to :workspace.', ['workspace' => $organization->name])">
            <div class="divide-y divide-primary bg-primary">
                @foreach($organization->members as $member)
                    ...
                    @if($canManage && $member->id !== $organization->owner_id)<form method="POST" action="{{ route('organizations.members.update', $member) }}" class="flex gap-2">@csrf @method('PATCH')<select name="role" class="input secondary rounded-sm">@foreach(\App\Models\Organization::ROLES as $role)<option value="{{ $role }}" @selected($member->pivot->role === $role)>{{ ucfirst($role) }}</option>@endforeach</select><button type="submit" class="button primary">{{ __('Save') }}</button></form><form method="POST" action="{{ route('organizations.members.destroy', $member) }}">@csrf @method('DELETE')<button type="submit" class="button tertiary">{{ __('Remove') }}</button></form>@endif</div>
                @endforeach
            </div>
        </x-forms.section>
        <div class="space-y-6">
            @if($canManage)
                <x-forms.section :title="__('Security policy')" :description="__('Enforce access requirements for everyone in this workspace.')">
                    <form method="POST" action="{{ route('organizations.security-policy.update') }}" class="space-y-4 bg-primary p-5">@csrf @method('PATCH')
                        <label class="block"><span class="mb-1 block text-sm font-bold text-primary">{{ __('Allowed IP ranges') }}</span><textarea name="allowed_ip_ranges" rows="3" class="input secondary w-full rounded-sm font-mono" placeholder="203.0.113.10/32&#10;2001:db8::/48">{{ old('allowed_ip_ranges', implode("\n", $organization->allowed_ip_ranges ?? [])) }}</textarea><span class="mt-1 block text-xs text-secondary">{{ __('Leave empty for any network. Your current IP must be included before saving.') }}</span></label>
                        <label class="block"><span class="mb-1 block text-sm font-bold text-primary">{{ __('Member email domains') }}</span><input name="allowed_email_domains" value="{{ old('allowed_email_domains', implode(', ', $organization->allowed_email_domains ?? [])) }}" class="input secondary w-full rounded-sm" placeholder="example.com, agency.test"></label>
                        ...
                        <label class="block"><span class="mb-1 block text-sm text-secondary">{{ __('Issuer URL') }}</span><input type="url" name="sso_issuer" value="{{ old('sso_issuer', $organization->sso_configuration['issuer'] ?? '') }}" class="input secondary w-full rounded-sm" placeholder="https://identity.example.com"></label>
                        <label class="block"><span class="mb-1 block text-sm text-secondary">{{ __('Client ID') }}</span><input name="sso_client_id" value="{{ old('sso_client_id', $organization->sso_configuration['client_id'] ?? '') }}" class="input secondary w-full rounded-sm"></label>
                        <label class="block"><span class="mb-1 block text-sm text-secondary">{{ __('Client secret') }}</span><input type="password" name="sso_client_secret" class="input secondary w-full rounded-sm" autocomplete="new-password" placeholder="{{ filled($organization->sso_configuration['client_secret'] ?? null) ? __('Stored — leave blank to keep') : '' }}"></label>
            ...
            @endif
            @if($canManage)<x-forms.section :title="__('Invite member')" :description="__('Invitations expire after seven days.')"><form method="POST" action="{{ route('organizations.invitations.store') }}" class="space-y-4 bg-primary p-5">@csrf<label class="block"><span class="mb-1 block text-sm text-secondary">{{ __('Email') }}</span><input required type="email" name="email" class="input secondary rounded-sm"></label><label class="block"><span class="mb-1 block text-sm text-secondary">{{ __('Role') }}</span><select name="role" class="input secondary rounded-sm">@foreach(\App\Models\Organization::ROLES as $role)<option value="{{ $role }}">{{ ucfirst($role) }}</option>@endforeach</select></label><button type="submit" class="button primary" @disabled(!$memberUsage['allowed'])>{{ __('Send invitation') }}</button>@unless($memberUsage['allowed'])<p class="text-sm text-secondary">{{ __('Your plan’s member limit has been reached.') }} <a href="{{ route('billing.index') }}" class="font-bold underline">{{ __('Upgrade') }}</a></p>@endunless</form></x-forms.section>@endif
    ...
    @if($organization->owner->is(auth()->user()))
        <section class="mt-8 rounded-2xl border border-red-200 bg-red-50 p-5">
            ...
            <form method="POST" action="{{ route('organizations.destroy', $organization) }}" class="mt-5 grid gap-3 sm:grid-cols-2">
                @csrf @method('DELETE')
                <label class="block"><span class="mb-1 block text-sm text-red-900">{{ __('Type “:name”', ['name' => $organization->name]) }}</span><input name="confirmation" class="input secondary w-full rounded-sm" required></label>
                @if(auth()->user()->hasLocalPassword())<label class="block"><span class="mb-1 block text-sm text-red-900">{{ __('Current password') }}</span><input type="password" name="current_password" autocomplete="current-password" class="input secondary w-full rounded-sm" required></label>@endif
                @if(auth()->user()->twoFactorEnabled())<label class="block"><span class="mb-1 block text-sm text-red-900">{{ __('Authenticator or recovery code') }}</span><input name="code" autocomplete="one-time-code" class="input secondary w-full rounded-sm font-mono" required></label>@endif
