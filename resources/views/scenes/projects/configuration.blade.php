    ...
    @else
        ...
        <x-ui.card class="mt-6 p-5" aria-labelledby="environment-overview-heading">
                    ...
                    @foreach($environmentOverview as $environment)
                        <article data-configuration-environment class="rounded-xl border border-line bg-surface-muted p-4">
                            ...
                            <dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4">
                                ...
                                <div><dt class="text-xs font-bold uppercase tracking-wide text-muted">{{ __('Configuration') }}</dt><dd class="mt-1 text-ink">{{ $environment->processCount }} {{ __('process(es)') }} · {{ $environment->resourceCount }} {{ __('resource(s)') }} · {{ $environment->variableCount }} {{ __('variable(s)') }}<span class="mt-1 block text-xs text-muted">{{ $environment->secretCount }} {{ __('secret value(s) masked') }}</span></dd></div>
                                <div class="sm:col-span-2"><dt class="text-xs font-bold uppercase tracking-wide text-muted">{{ __('Recorded dependencies') }}</dt><dd class="mt-1"><ul class="space-y-1">@foreach($environment->dependencies as $dependency)<li><span class="font-medium text-ink">{{ ucfirst($dependency['kind']) }}:</span> {{ $dependency['name'] }} <span class="text-muted">· {{ str_replace('_', ' ', $dependency['status']) }} · {{ $dependency['detail'] }}</span></li>@endforeach</ul></dd></div>
                            </dl>
                        </article>
                    @endforeach
                </div>
            @else
                <p class="mt-4 text-sm text-muted">{{ __('No environments have been recorded yet.') }}</p>
            @endif
            @if($environmentOverview->count() > 1)
                ...
                <form method="GET" action="{{ route('projects.configuration.compare', $project) }}" class="mt-5 grid gap-3 border-t border-line pt-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
                    <label><span class="mb-1 block text-xs font-bold uppercase text-muted">{{ __('Compare from') }}</span><select name="from_environment_id" class="ui-input w-full rounded-lg" required>@foreach($environmentOverview as $environment)<option value="{{ $environment->id }}" @selected($fromEnvironmentId === $environment->id)>{{ $environment->name }}</option>@endforeach</select></label>
                    <label><span class="mb-1 block text-xs font-bold uppercase text-muted">{{ __('Compare to') }}</span><select name="to_environment_id" class="ui-input w-full rounded-lg" required>@foreach($environmentOverview as $environment)<option value="{{ $environment->id }}" @selected($toEnvironmentId === $environment->id)>{{ $environment->name }}</option>@endforeach</select></label>
                    ...
                    <div class="mt-4 divide-y divide-line rounded-lg border border-line" aria-label="{{ __('Safe recorded environment differences') }}">
                        @foreach($comparison->differences as $difference)
                            <article data-configuration-comparison-field class="p-4">
                                ...
                                <dl class="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt class="text-xs font-bold uppercase tracking-wide text-muted">{{ $comparison->from->name }}</dt><dd class="mt-1 text-muted">{{ $difference['from'] }}</dd></div><div><dt class="text-xs font-bold uppercase tracking-wide text-muted">{{ $comparison->to->name }}</dt><dd class="mt-1 text-muted">{{ $difference['to'] }}</dd></div></dl>
                            </article>
                        @endforeach
                    </div>
        ...
        <details class="ui-card mt-6 p-5">
            ...
            <div class="mt-4 grid gap-6 lg:grid-cols-3">
                <section><h2 class="font-bold text-ink">{{ __('Websites · placements') }}</h2><ul class="mt-2 space-y-2">@forelse($websites as $site)<li class="text-muted">#{{ $site->id }} · {{ $site->name }} · {{ $site->url }}</li>@empty<li class="text-muted">{{ __('No websites available.') }}</li>@endforelse</ul>{{ $websites->withQueryString()->links() }}</section>
