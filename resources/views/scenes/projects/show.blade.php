        ...
        @foreach($project->environments as $environment)
            ...
            <section class="overflow-hidden rounded-2xl border border-primary bg-primary shadow-xs">
                <div class="flex flex-wrap items-center gap-4 border-b border-primary px-5 py-4">
                    ...
                    <div class="flex flex-wrap gap-2 text-xs"><span class="rounded-lg bg-secondary px-3 py-2 text-secondary">{{ $environment->server?->label ?? __('No server') }}</span><span class="rounded-lg bg-secondary px-3 py-2 text-secondary">{{ $environment->website?->name ?? __('No site') }}</span><span class="rounded-lg bg-secondary px-3 py-2 font-bold text-secondary">{{ $environment->minimum_replicas }}–{{ $environment->maximum_replicas }}×</span></div>
                </div>
                ...
                <div class="grid gap-px bg-secondary lg:grid-cols-3">
                    <details class="group bg-primary p-5">
                        ...
                        <form method="POST" action="{{ route('environments.update', $environment) }}" class="mt-4 grid gap-3 sm:grid-cols-2">@csrf @method('PATCH')
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Name') }}</span><input name="name" value="{{ $environment->name }}" class="input secondary rounded-sm" required></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Type') }}</span><select name="type" class="input secondary rounded-sm">@foreach(\App\Models\Environment::TYPES as $type)<option value="{{ $type }}" @selected($environment->type === $type)>{{ ucfirst($type) }}</option>@endforeach</select></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Branch') }}</span><input name="branch" value="{{ $environment->branch }}" class="input secondary rounded-sm" required></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Runtime') }}</span><select name="runtime_type" class="input secondary rounded-sm">@foreach(\App\Models\Environment::RUNTIME_TYPES as $runtime)<option value="{{ $runtime }}" @selected(($environment->runtime_type ?: 'php') === $runtime)>{{ ucfirst($runtime) }}</option>@endforeach</select></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Runtime version') }}</span><input name="runtime_version" value="{{ $environment->runtime_version }}" placeholder="20" class="input secondary rounded-sm"></label>
                            <label class="sm:col-span-2"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Build command') }}</span><input name="build_command" value="{{ $environment->build_command }}" placeholder="npm run build" class="input secondary rounded-sm font-mono"></label>
                            <label class="sm:col-span-2"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Start command') }}</span><input name="start_command" value="{{ $environment->start_command }}" placeholder="npm start" class="input secondary rounded-sm font-mono"></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Application port') }}</span><input type="number" min="1" max="65535" name="container_port" value="{{ $environment->container_port }}" placeholder="3000" class="input secondary rounded-sm"></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Dockerfile path') }}</span><input name="dockerfile_path" value="{{ $environment->dockerfile_path }}" placeholder="Dockerfile" class="input secondary rounded-sm font-mono"></label>
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Server') }}</span><select name="server_id" class="input secondary rounded-sm"><option value="">{{ __('None') }}</option>@foreach($servers as $server)<option value="{{ $server->id }}" @selected($environment->server_id === $server->id)>{{ $server->label }}</option>@endforeach</select></label>
                            <label class="sm:col-span-2"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Website') }}</span><select name="website_id" class="input secondary rounded-sm"><option value="">{{ __('None') }}</option>@foreach($websites as $website)<option value="{{ $website->id }}" @selected($environment->website_id === $website->id)>{{ $website->name }}</option>@endforeach</select></label>
                            @if($featureAccess['hibernation'])<label class="sm:col-span-2"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Hibernate after inactivity') }}</span><select name="hibernate_after_minutes" class="input secondary rounded-sm"><option value="">{{ __('Never') }}</option>@foreach([5, 15, 30, 60, 120, 1440] as $minutes)<option value="{{ $minutes }}" @selected($environment->hibernate_after_minutes === $minutes)>{{ trans_choice(':count minute|:count minutes', $minutes, ['count' => $minutes]) }}</option>@endforeach</select></label>@endif
                    ...
                    <details class="group bg-primary p-5">
                        ...
                        @can('update', $environment)
                            <form method="POST" action="{{ route('environments.deployment-controls.update', $environment) }}" class="mt-4 space-y-4">@csrf @method('PATCH')
                                ...
                                <input name="deployment_lock_reason" maxlength="500" value="{{ old('deployment_lock_reason', $environment->deployment_lock_reason) }}" class="input secondary rounded-sm" placeholder="{{ __('Reason for the lock (optional)') }}">
                    ...
                    <details class="group bg-primary p-5">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-3">
                            <label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Minimum') }}</span><input value="{{ $environment->minimum_replicas }}" class="input secondary rounded-sm" disabled></label>
                    ...
                    <details class="group bg-primary p-5">
                        <summary class="flex cursor-pointer list-none items-center justify-between font-bold text-primary"><span>{{ __('Encrypted variables') }} <span class="text-secondary">({{ $environment->variables->count() }})</span></span><span class="text-secondary group-open:rotate-45">+</span></summary>
                        ...
                        @can('update', $environment)<form method="POST" action="{{ route('environments.variables.store', $environment) }}" class="mt-4 space-y-3">@csrf<input name="key" placeholder="APP_SETTING" class="input secondary rounded-sm font-mono" required><textarea name="value" rows="2" placeholder="{{ __('Encrypted value') }}" class="input secondary rounded-sm" required></textarea><div class="grid gap-3 sm:grid-cols-2"><label><span class="block text-xs font-bold uppercase text-secondary">{{ __('Scope') }}</span><select name="scope" class="input secondary mt-1 rounded-sm"><option value="runtime">{{ __('Runtime only') }}</option><option value="build">{{ __('Build only') }}</option><option value="all">{{ __('Build and runtime') }}</option></select></label><label><span class="block text-xs font-bold uppercase text-secondary">{{ __('Rotate by (optional)') }}</span><input type="date" name="rotation_due_at" min="{{ now()->addDay()->toDateString() }}" class="input secondary mt-1 rounded-sm"></label></div><label class="flex items-center gap-2"><input type="checkbox" name="is_secret" value="1" checked><span class="text-sm text-secondary">{{ __('Mask as secret') }}</span></label><button type="submit" class="button primary">{{ __('Save new version') }}</button></form>@endcan
                    </details>
                </div>
