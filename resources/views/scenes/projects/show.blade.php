<x-layouts.app>
        ...
        @foreach($project->environments as $environment)
            ...
            <section class="ui-card overflow-hidden shadow-xs" aria-labelledby="environment-{{ $environment->id }}-heading">
                ...
                <div class="grid gap-px bg-secondary lg:grid-cols-3">
                    ...
                    <details id="environment-{{ $environment->id }}-variables" class="group bg-primary p-5" @if($variablesOpen) open @endif>
                        ...
                        @can('update', $environment)<form method="POST" action="{{ route('environments.variables.store', $environment) }}" class="mt-4 space-y-3">@csrf<input type="hidden" name="_environment_id" value="{{ $environment->id }}"><input type="hidden" name="_environment_panel" value="variables"><input name="key" placeholder="APP_SETTING" class="input secondary rounded-lg font-mono" required><textarea name="value" rows="2" placeholder="{{ __('Encrypted value') }}" class="input secondary rounded-lg" required></textarea><div class="grid gap-3 sm:grid-cols-2"><label><span class="block text-xs font-bold uppercase text-secondary">{{ __('Scope') }}</span><select name="scope" class="input secondary mt-1 rounded-lg"><option value="runtime">{{ __('Runtime only') }}</option><option value="build">{{ __('Build only') }}</option><option value="all">{{ __('Build and runtime') }}</option></select></label><label><span class="block text-xs font-bold uppercase text-secondary">{{ __('Rotate by (optional)') }}</span><input type="date" name="rotation_due_at" min="{{ now()->addDay()->toDateString() }}" class="input secondary mt-1 rounded-lg"></label></div><label class="flex items-center gap-2"><input type="checkbox" name="is_secret" value="1" checked><span class="text-sm text-secondary">{{ __('Mask as secret') }}</span></label><button type="submit" class="button primary">{{ __('Save new version') }}</button></form>@endcan
                    </details>
                </div>

                <div class="grid gap-5 border-t border-primary p-5 lg:grid-cols-2">
                    <details id="environment-{{ $environment->id }}-processes" class="ui-card ui-card--muted group p-4" @if($processesOpen) open @endif>
                        ...
                        @can('update', $environment)@if($featureAccess['workers'])<form method="POST" action="{{ route('environments.processes.store', $environment) }}" class="mt-4 grid gap-3 sm:grid-cols-2">@csrf<input type="hidden" name="_environment_id" value="{{ $environment->id }}"><input type="hidden" name="_environment_panel" value="processes"><input name="name" placeholder="queue" class="input secondary rounded-lg" required><select name="type" class="input secondary rounded-lg"><option value="worker">{{ __('Worker') }}</option><option value="scheduler">{{ __('Scheduler') }}</option></select><input name="command" placeholder="php artisan queue:work" class="input secondary rounded-lg font-mono sm:col-span-2" required><input type="number" name="replicas" min="1" max="20" value="1" class="input secondary rounded-lg"><select name="restart_policy" class="input secondary rounded-lg"><option value="always">{{ __('Always restart') }}</option><option value="on-failure">{{ __('Restart on failure') }}</option><option value="no">{{ __('Never restart') }}</option></select><input type="number" name="restart_delay_seconds" min="0" max="300" value="5" class="input secondary rounded-lg" aria-label="{{ __('Restart delay seconds') }}"><input type="hidden" name="is_enabled" value="1"><button type="submit" class="button primary">{{ __('Save process') }}</button></form>@else<p class="mt-4 text-sm text-secondary">{{ __('Available on Starter and higher.') }} <a href="{{ route('pricing') }}" class="font-bold text-ternary">{{ __('View plans') }}</a></p>@endif @endcan
                    </details>
    ...
    @php($addEnvironmentOpen = old('_environment_form') === 'add')

    <div class="mt-5 grid gap-5 xl:grid-cols-2">
        @if($canDeploy)
            <details id="add-environment" class="ui-card p-5" @if($addEnvironmentOpen) open @endif><summary class="cursor-pointer font-black text-primary">{{ __('Add environment') }}</summary><p class="mt-2 text-sm text-secondary">{{ __('Create a separate runtime for a branch, then attach infrastructure and source control.') }}</p><form method="POST" action="{{ route('environments.store', $project) }}" class="mt-4 grid gap-3 sm:grid-cols-2">@csrf<input type="hidden" name="_environment_form" value="add"><input name="name" placeholder="Staging" class="input secondary rounded-lg" required><select name="type" class="input secondary rounded-lg">@foreach(\App\Models\Environment::TYPES as $type)<option value="{{ $type }}">{{ ucfirst($type) }}</option>@endforeach</select><input name="branch" placeholder="develop" class="input secondary rounded-lg sm:col-span-2" required><input type="hidden" name="is_protected" value="0"><input type="hidden" name="requires_deployment_approval" value="0"><x-ui.button type="submit" variant="primary" class="sm:col-span-2">{{ __('Create environment') }}</x-ui.button></form></details>
        @endif
