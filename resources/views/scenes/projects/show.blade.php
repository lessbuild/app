            ...
            <section class="ui-card overflow-hidden shadow-xs" aria-labelledby="environment-{{ $environment->id }}-heading">
                ...
                <div class="grid gap-px bg-secondary lg:grid-cols-3">
                    ...
                    <section id="environment-{{ $environment->id }}-variables" class="bg-primary p-5" aria-labelledby="environment-{{ $environment->id }}-variables-heading">
                        ...
                        @can('update', $environment)
                            <x-dialogs.modal :id="$variableDialogId" :title="__('Add encrypted variable')" :description="__('Save a new version without exposing the value in page content or URLs.')" :open="$variableDialogOpen">
                                <form method="POST" action="{{ route('environments.variables.store', $environment) }}" class="space-y-3">@csrf<input type="hidden" name="_environment_id" value="{{ $environment->id }}"><input type="hidden" name="_environment_panel" value="variables"><label class="block"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Key') }}</span><input name="key" value="{{ old('key') }}" placeholder="APP_SETTING" class="input secondary w-full rounded-lg font-mono" required><x-forms.errors name="key" /></label><label class="block"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Value') }}</span><textarea name="value" rows="3" placeholder="{{ __('Encrypted value') }}" class="input secondary w-full rounded-lg" required></textarea><x-forms.errors name="value" /></label><div class="grid gap-3 sm:grid-cols-2"><label><span class="block text-xs font-bold uppercase text-secondary">{{ __('Scope') }}</span><select name="scope" class="input secondary mt-1 w-full rounded-lg"><option value="runtime" @selected(old('scope', 'runtime') === 'runtime')>{{ __('Runtime only') }}</option><option value="build" @selected(old('scope') === 'build')>{{ __('Build only') }}</option><option value="all" @selected(old('scope') === 'all')>{{ __('Build and runtime') }}</option></select><x-forms.errors name="scope" /></label><label><span class="block text-xs font-bold uppercase text-secondary">{{ __('Rotate by (optional)') }}</span><input type="date" name="rotation_due_at" value="{{ old('rotation_due_at') }}" min="{{ now()->addDay()->toDateString() }}" class="input secondary mt-1 w-full rounded-lg"><x-forms.errors name="rotation_due_at" /></label></div><label class="flex items-center gap-2"><input type="checkbox" name="is_secret" value="1" @checked(old('is_secret', '1') === '1')><span class="text-sm text-secondary">{{ __('Mask as secret') }}</span></label><x-ui.button type="submit" variant="primary">{{ __('Save new version') }}</x-ui.button></form>
                            </x-dialogs.modal>
                        @endcan
                    </section>
                </div>
                ...
                <div class="grid gap-5 border-t border-primary p-5 lg:grid-cols-2">
                    <section id="environment-{{ $environment->id }}-processes" class="ui-card ui-card--muted p-4" aria-labelledby="environment-{{ $environment->id }}-processes-heading">
                        ...
                        @can('update', $environment)
                            @if($featureAccess['workers'])
                                <x-dialogs.modal :id="$processDialogId" :title="__('Add worker or scheduler')" :description="__('Define an encrypted process command for the next deployment.')" :open="$processDialogOpen">
                                    <form method="POST" action="{{ route('environments.processes.store', $environment) }}" class="grid gap-3 sm:grid-cols-2">@csrf<input type="hidden" name="_environment_id" value="{{ $environment->id }}"><input type="hidden" name="_environment_panel" value="processes"><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Name') }}</span><input name="name" value="{{ old('name') }}" placeholder="queue" class="input secondary w-full rounded-lg" required><x-forms.errors name="name" /></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Type') }}</span><select name="type" class="input secondary w-full rounded-lg"><option value="worker" @selected(old('type', 'worker') === 'worker')>{{ __('Worker') }}</option><option value="scheduler" @selected(old('type') === 'scheduler')>{{ __('Scheduler') }}</option></select><x-forms.errors name="type" /></label><label class="sm:col-span-2"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Command') }}</span><input name="command" value="{{ old('command') }}" placeholder="php artisan queue:work" class="input secondary w-full rounded-lg font-mono" required><x-forms.errors name="command" /></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Replicas') }}</span><input type="number" name="replicas" min="1" max="20" value="{{ old('replicas', 1) }}" class="input secondary w-full rounded-lg" required><x-forms.errors name="replicas" /></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Restart policy') }}</span><select name="restart_policy" class="input secondary w-full rounded-lg"><option value="always" @selected(old('restart_policy', 'always') === 'always')>{{ __('Always restart') }}</option><option value="on-failure" @selected(old('restart_policy') === 'on-failure')>{{ __('Restart on failure') }}</option><option value="no" @selected(old('restart_policy') === 'no')>{{ __('Never restart') }}</option></select><x-forms.errors name="restart_policy" /></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Restart delay seconds') }}</span><input type="number" name="restart_delay_seconds" min="0" max="300" value="{{ old('restart_delay_seconds', 5) }}" class="input secondary w-full rounded-lg" aria-label="{{ __('Restart delay seconds') }}" required><x-forms.errors name="restart_delay_seconds" /></label><input type="hidden" name="is_enabled" value="1"><x-ui.button type="submit" variant="primary" class="sm:col-span-2">{{ __('Save process') }}</x-ui.button></form>
                                </x-dialogs.modal>
                            @else
    ...
    <div class="mt-5 grid gap-5 xl:grid-cols-2">
        @if($canDeploy)
            <section id="add-environment" class="ui-card p-5" aria-labelledby="add-environment-heading">
                ...
                <x-dialogs.modal :id="$addEnvironmentDialogId" :title="__('Add environment')" :description="__('Create a separate runtime for a branch, then attach infrastructure and source control.')" :open="$addEnvironmentOpen">
                    <form method="POST" action="{{ route('environments.store', $project) }}" class="grid gap-3 sm:grid-cols-2">@csrf<input type="hidden" name="_environment_form" value="add"><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Name') }}</span><input name="name" value="{{ old('name') }}" placeholder="Staging" class="input secondary w-full rounded-lg" required><x-forms.errors name="name" /></label><label><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Type') }}</span><select name="type" class="input secondary w-full rounded-lg">@foreach(\App\Models\Environment::TYPES as $type)<option value="{{ $type }}" @selected(old('type', 'staging') === $type)>{{ ucfirst($type) }}</option>@endforeach</select><x-forms.errors name="type" /></label><label class="sm:col-span-2"><span class="mb-1 block text-xs font-bold uppercase text-secondary">{{ __('Branch') }}</span><input name="branch" value="{{ old('branch') }}" placeholder="develop" class="input secondary w-full rounded-lg" required><x-forms.errors name="branch" /></label><input type="hidden" name="is_protected" value="0"><input type="hidden" name="requires_deployment_approval" value="0"><x-ui.button type="submit" variant="primary" class="sm:col-span-2">{{ __('Create environment') }}</x-ui.button></form>
                </x-dialogs.modal>
            </section>
        @endif
