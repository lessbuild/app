        ...
        <div class="mx-auto max-w-7xl">
            ...
            <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                @foreach($plans as $key => $plan)
                    <x-ui.card @class(['relative flex flex-col p-7 shadow-xs', 'border-ternary ring-2 ring-ternary' => $key === 'pro'])>
                        ...
                        <ul class="my-7 flex-1 space-y-3 text-secondary">@foreach($plan['features'] as $feature)<li class="flex gap-3"><span class="font-bold text-ternary">✓</span>{{ $feature }}</li>@endforeach<li class="flex gap-3"><span class="font-bold text-ternary">✓</span>{{ number_format($plan['limits']['api_requests_per_minute']) }} {{ __('API requests per minute') }}</li></ul>
