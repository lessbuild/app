        ...
        <div class="mx-auto max-w-7xl">
            ...
            <div id="pricing-plans" class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                @foreach($plans as $key => $plan)
                    ...
                    @endphp
                    <x-ui.card data-pricing-plan @class(['relative flex flex-col p-7 shadow-xs', 'border-ternary ring-2 ring-ternary' => $key === 'pro'])>
                        ...
                        <ul class="my-7 flex-1 space-y-3 text-secondary">@foreach($visibleFeatures as $feature)<li class="flex gap-3"><span class="font-bold text-ternary">✓</span>{{ $feature }}</li>@endforeach</ul>
                        <details class="ui-responsive-details mt-3 border-t border-primary pt-3" open data-responsive-details data-responsive-details-mobile-open="false">
                            <summary class="cursor-pointer list-none text-sm font-bold text-ternary">{{ __('See all features and limits') }}</summary>
                            <div class="ui-responsive-details__content mt-3 space-y-3 text-sm text-secondary">
                                @foreach($additionalFeatures as $feature)<p class="flex gap-3"><span class="font-bold text-ternary">✓</span>{{ $feature }}</p>@endforeach
                                <p class="flex gap-3"><span class="font-bold text-ternary">✓</span>{{ number_format($plan['limits']['api_requests_per_minute']) }} {{ __('API requests per minute') }}</p>
                            </div>
                        </details>
